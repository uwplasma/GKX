...
| Q23 | supplied states below the runtime | Q19 follow-up: `run_runtime_linear` and the init/restart file now mask the off-chain rows at intake (#247), but the library entry points underneath still accept whatever state they are given — `gkx.prepare`, a nonlinear `simulation.run(initial_state)` and the objective adjoints. Decide one contract for them (mask, or reject with an explicit error) and test it; #247 showed an unmasked nonlinear state aliases back onto the chain rows through the bracket at 4.5e-2 relative | Q19 merged | CPU |
