...
| Q27 | `perf/linked-gather-row-order` | N3 follow-up, the blocker to adopting the half layout — the full-vs-half ledger (this PR) measures the RK step at +43 to +57% bytes on a *linked* deck while the RHS is −53%, with gathers falling and copies/transposes rising 54–94%; a chain-free control wins on both graphs, so the cost is the linked-chain gather materializing its operand instead of fusing. `_flatten_linked_fft_state` keeps the state's own row order and translates the chain maps with `_state_mode_rows` precisely to avoid that, and the recognition is lost when the ky extent goes 32→17 or 64→33. Build the chain maps directly in state row order on a half grid (or pad `Nyc`), re-run `run_ledgers.sh`, and only then decide the default flip | Q10 switch merged | CPU |
