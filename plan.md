...
| Q24 | mode-weight duplication (N3 prerequisite) | #248 found the ky weight rule written three times: `_transport_mode_weight`, `_hermitian_mode_weight` (whose even-grid Nyquist row takes weight 2 on a half axis — a recorded contract violation, unreachable today only because dealiasing zeroes that row) and a third copy in `diagnostics/quasilinear_transport`. Give them one owner beside `gkx.core_ky_layout` before the state becomes half-spectrum. Also pin `--xla_cpu_multi_thread_eigen=false` in the float32 identity harness: #248 showed two runs of unmodified `main` differ by 1.4e-10 on the f32 nonlinear RHS under the multithreaded XLA:CPU FFT thunk | Q10 partial merged | CPU |
