...
| Q10 | `perf/ky-half-spectrum-layout` | §5.3 N3 — **partial, #248**: the contract is written and owned (`gkx.core_ky_layout`), the five disagreeing hand-written completions are one rule, the bracket's `ky ≥ 0` primitive is split out as the seam the switch will call, and 171 cases pin the layout over even and odd `Ny`. **The evolved state is still two-sided**, so none of the profiled 41.9% is recovered and no speed-up is claimed; every HLO count and byte total is unchanged from main, and the RHS, VJP and 100-step trajectory gates are bitwise in f32 and x64. Found and fixed on the way: an odd-`Ny` raw restart expanded to `Ny-1` rows, and `PreparedSimulation.state_shape` advertised `Nyc` while the runtime allocates `Ny`. Left for the follow-up row: the hot-path switch itself and the seven blockers it must clear (linked-chain `naky` and the conjugate restore, the two-sided dealias mask, `_transport_mode_weight`, the `two_sided=True` projector, the sharded ky divisibility check, the full-complex `ifft2` output paths, and the Nyquist row of `_hermitian_mode_weight`) — **two of those seven are cleared by Q24 (#254)**: the weight rule has one owner, the half-axis Nyquist weight is the contract's, and grids and caches carry the `ny_full` the switch needs; what is left of `_transport_mode_weight` is the two-sided representative convention, which is the switch's own decision | Q4 merged (N0 ledger), Q9 measured | CPU, then GPU |
