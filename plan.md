...
| Q9 | `perf/batched-chain-fft` | §5.3 N2 — **done, #243**: shared per-class transforms adopted (streaming and the \|kz\| hypercollision branch share one FFT/IFFT per chain class, the exact floor since each chain length has its own frequencies; the chain gather keeps the state's own axis order): RHS at 32×32×24 FFT 23→13 and bytes 46.3→25.0 MB, every ledgered graph −43 to −48% bytes, 100-step trajectories bitwise in f32 and x64. Timings on a loaded office host were inconclusive, so the adoption rests on that load-independent evidence; the idle-host A/B/A/B rerun (2026-09-18 log entry, arm order rotated per block) measures the shared transform 0.88× base on the RHS gradient and 0.94× on the window gradient but 1.03× on the rk3 scan at both grids and 1.22× on the standalone RHS at 32×32×24 — faster on the adjoint path, a measured forward-path regression that does not pass this row's own registered timing gate, follow-up proposed as Q22 | Q4 merged | CPU, office idle cores for timing |
| Q10 | `perf/ky-half-spectrum-layout`, `perf/ky-half-spectrum-switch`, `perf/ky-half-default` | §5.3 N3 — **default flipped in a draft PR, not yet merged; three items outstanding (end of row)** (stage 1 #248, switch #258, byte metric #259, default flip this PR). History of the row follows. Stage 1 wrote the contract and the bracket seam. This row clears the seven blockers and makes a half-spectrum state work end to end through the operator layer — `build_spectral_grid(..., ky_layout="half")` gives `Nyc = 1 + Ny//2` rows, the bracket computes on them and does not widen, and the per-stage projector is the **identity**, not a cheaper copy. Blockers: `naky` now counts the dealiased rows of the two-sided axis while the flat chain index keeps the stored count; the conjugate restore, the cover mask's mirror and the end-damping mirror are *inapplicable* on a half axis, not merely skipped (`(-j) % Nyc` names an unrelated positive row); `half_twothirds_mask` states the half rows instead of slicing; the compressed projector takes the layout; the sharded ky refusal names the layout its extent came from; the real-space writers take `irfft2`; `kperp2_max` takes its cutoff row from `ny_full` (off the stored count it halved the cutoff and inflated every hyperdiffusion rate by `4**p_hyper_kperp` with no shape error). **Three latent correctness defects found and fixed, independent of the layout**: `kperp2_max` (two independent copies) took its dealias cutoff row from the stored row count, which on a half grid halves the cutoff and inflates every hyperdiffusion rate by `4**p_hyper_kperp` — 16× at the default `p_hyper_kperp = 2` — with no shape error; the restart writer took the dealiased block's length the same way and would write about a third of the band without complaint; and the seeded initial band had the same defect. One root cause: the identifier `ny` was doing two jobs, "how long is the physical axis" and "how many rows does this array hold", and they were the same number so nothing distinguished them. **Flux convention decided** (Q24's hand-off): an even grid's Nyquist row is a flux representative in *both* layouts at the self-conjugate weight — the kernel's explicit `i·ky` makes that row's contribution odd under a sign choice that is pure convention, and it is identically zero on any state representing a real field. **Identity**: 58/58 and 65/65 bitwise against `origin/main` in f32 and x64, `max_rel` exactly 0, VJPs and `heat_flux_t`/`Wg_t`/`Wphi_t` included. **Ledger, full vs half**: RHS bytes −53.3% (32) and −52.5% (64), `reverse` 3→0 (the completion itself), concatenates −22%/−18%; but the **RK step regresses**, bytes +42.7%/+57.0% (rk3) and +18.6%/+29.6% (rk4), copies and transposes +54–94% while gathers *fall* 41→33. Cause measured, not guessed: chain topology is identical (same five classes), and a `boundary="periodic"` control with no chains wins on both graphs (RHS −23.4%, rk3 −10.5%) — the linked-chain gather loses its fused lowering when the state's ky extent stops being the grid's power of two, the mechanism #248 recorded at `_reverse_from_one`. **So the default is not flipped**, and no speed-up is claimed. Left: the runtime grid flip, the diagnostics/NetCDF condensation chain (`_condense_ky_for_output`, the `y` dimension), and the eigen branch-selection question (a half axis carries no conjugate `λ*` copies) **Adoption gate update (Q27, #259):** the RK-step regression above is an artefact of the byte metric, not of the layout. Re-measured on the bytes that own an output buffer, the half layout wins all twelve graphs — rk3 −73.5%/−72.4% and rk4 −71.4%/−70.3% against a state only 46.9% smaller — and XLA's buffer assignment agrees. **The cost is no longer a reason to wait.** What is left before the flip is this row's own list: the runtime/workflow grid flip and `PreparedSimulation.state_shape`, the diagnostics/NetCDF condensation chain, the eigen branch-selection decision, and one idle-host wall-clock A/B for the strided in-fusion reads Q27 could not price — a confirmation, not a gate. **Default flip (this PR):** `GridConfig.ky_layout` is a deck key, `[grid] ky_layout`, defaulting to `"half"`; `"full"` is the documented opt-out. Every grid a run builds — solver, diagnostics, restart writer, NetCDF writer — reads the key, so they cannot land on different axes, and `PreparedSimulation.state_shape` reports the rows the run allocates. The four items this row left are closed. (1) Runtime/workflow grid flip: done as above. (2) Diagnostics/NetCDF condensation: the writer divides the pair weight back out of `Phi2`/`Wg`/`Wphi`/`Wapar`/`TurbulentHeating`, takes `ny_full` for every dealiased count, and folds the fixed-`kx` ky reduction onto `(ky,kx)`↔`(-ky,-kx)` pairs (without the fold `Phi2_kxt` was 11% and `Wphi_kxst` 13% wrong, returning the kx-reflected spectrum). (3) Eigen branch selection: every eigen route selects one `ky` row before it builds an operator, so no conjugate pair was ever in the spectrum on either axis; only the Nyquist row can land on the other sign, and it is above the dealias cutoff. (4) Wall clock, benchmark host, tree `38d7c4277`, six pinned cores with idle HT siblings, arm order rotated, 4 blocks × 7 reps: RK3 step **0.54×** (32×32×24) and **0.60×** (64×64×24), RHS 0.49×/0.63×, RHS VJP 0.83×/0.96×; the eager window VJP is 2.31× at 32×32×24; that route recompiles 13 XLA modules on every call (#264), and its compile/execution split is not yet measured on an idle host. **Nothing published moved**: the Cyclone parity generator's gamma and omega at all eight tracked ky, and the example's certified Krylov eigenvalue, are bitwise equal to `main`; the nonlinear NetCDF bundle is 221/225 variables bitwise in x64 with no shape change, the other four being `TurbulentHeating`, which is a cancellation residue at 3e-17 on this deck; restarts load across both axes in all four directions with max\|Δ\| = 0. Evidence: `plan/research/scripts/2026-09-20-q10-ky-half-default/`. **Outstanding before merge:** (a) the opt-out identity gate (`run_identity.sh`, `new_full` vs `main`, must be bitwise) — its first run was void because the harness pin missed the `GridConfig` instances baked into `RuntimeConfig`/`Case`/base-case defaults, now fixed and fail-closed; (b) two tests in `tests/unit/solvers/test_time_integrators.py` and one in `tests/integration/examples/test_examples.py` still build `Ny`-row states; (c) the integration suites after `test_examples.py` were not run to completion locally. | Q4 merged (N0 ledger), Q9 measured, Q24 merged, Q27 merged | CPU, then GPU |
...
| Q14 | `test/f32-window-gradients` | 11 finite-window gradient tests fail on unmodified `main` in float32 (found in #231); CI runs them only in x64 while f32 is the documented default precision — **done, #240**: no `src/` bug (f32 gradients match f64 to ≤5.7e-6); the float32 finite-difference references lost their step to rounding, so the ten matrix tests take an x64 finite-difference reference (rtol 1e-6 + 2.5e-5), the checkpoint-equivalence test a derived f32-only 5e-5, and a `python-floor` CI step runs them in f32 (passing on x86) | — | CPU |
...
| Q16 | `evidence/eigen-laguerre-spectrum` | §0.5 (ii) — **done, #252**: twelve certified adaptive eigenpairs at Nl 24/32/48 (residuals 9.3e-15–2.2e-10 against a 1e-9 gate) show the non-monotone small-ν ladders are **one slowly converging branch, not a branch change** (φ(z) overlap ≥ .991 on all 30 pairs, ≥ .9974 across the Nl32 dip; ω within a 2.2% band), and the only Nl-converged ladder, ν=1e-2, is the only one whose Laguerre spectrum decays monotonically (no interior maximum, 5.3–7.3 decades peak→cutoff), while every unconverged ladder carries a cutoff pile-up that merely relocates to 0.79–0.85 of each new Nl — collisionlessly the plateau is 1.33/1.02/0.54% per index in the upper quarter and γ still falls −24.7%/−26.2% per rung, so no collisionless Laguerre-converged mode exists at this ky up to Nl48. Every Q8 fit marked settled reproduces the eigenvalue to ≤0.01%; the two marked unsettled differ by 0.57% and 7.21%. The four Nl64 rungs were not reached (the collisionless key hit its 45-min cap) | Q6 merged, Q8 recorded | office CPU/GPU |
...
| Q18 | `fix/nonlinear-route-reference` | #239 found the prepared (captured-constant) and runtime (eager-scan) nonlinear routes differ at roundoff (f32 ≤2.1e-7, two near-cancelling diagnostics O(1)) — **done, #249**: one reference route, taken by giving `integrate_nonlinear_explicit_diagnostics_state` the prepared route's jit rather than the reverse, so both compile one graph and every output array is bitwise equal in f32 and x64 (12/12 gate cases, 79–80 arrays each; 8/8 of the new tests fail on `main`); declaring a tolerance was rejected because the near-cancelling turbulent-heating diagnostics are unbounded in relative terms (0.93 f32, 1.41 x64); cost is one-directional — runtime RK3 `copy` 226→85 and 89,062,692→89,479,380 bytes written (+0.47%), module literals 7,140→6,553,260 B, peak RSS over eight chunked calls 1,200→826 MB, compiles per call unchanged; VJP value and gradient were already bitwise on both routes and are unchanged; `--route runtime` now names the shared graph and `--route eager-scan` keeps the retired placement | Q13 merged | CPU |
| Q19 | `fix/off-chain-supplied-states` | Q6 follow-up (a) — **done, #247**: the runtime zeroes the off-chain rows of a user `initial_state` and of a restart/init file on linked decks, at intake only (the chains are closed under the linear *and* nonlinear right-hand side, so the time loop keeps them at exact zero with no per-step work); on the pilot this removes 99.13% of a supplied state's free energy (`Wg` 2.6946e-19 → 2.3444e-21, the runtime initial condition's own value) and the kx spectrum's rows 3/4/5, while the certified eigenpair and the explicit-time fit are unchanged to every printed digit; on a nonlinear grid the mask is not cosmetic — off-chain content aliases back onto the chain rows through the bracket (4.48e-02 relative on one RHS). Periodic and full-cover decks trace unchanged | Q6 merged | CPU |
...
| Q21 | `evidence/inner-solve-cost` | §5.1 L5 from Q7 — **done, #255**, no `src/` change: the gate fails again, but by 1.15–1.22× rather than Q7's 3.1×, because two levers are worth 2.8× together (160254 matvec-equivalents and 261 s against the same-session `adaptive` control's 195196 and 310 s, where Q7's configuration reproduces at 443930 and 748 s). Q7's own candidate — an inner tolerance ∝ the outer residual — does **not** certify (it stalls at 1.5e-3); the inexact-Krylov rule that relaxes the *late* steps instead gives 1.67× at an unchanged outer residual sequence, and an exact block-Thomas + Sherman–Morrison z-block solve a further 1.66×. Rejected: the Freitag–Spence tuned preconditioner (1.04× alone, negative combined) and harmonic Rayleigh–Ritz without inner solves (stalls at residual 0.4–0.5; it needs a basis ≳90% of n). `adaptive` stays the runtime default; the gate is now apply-bound, not iteration-bound. **Re-examined 2026-09-19 under the rewritten §5.1 gate (Q26, #257): neither lever is adoptable in `src/`, for a reason that is not the threshold.** Q21's arms measure a research harness whose shift-invert is a different algorithm from the shipped one — `gcrot` with recycling, the `pr3-cm` preconditioner, an unrestarted Arnoldi and a per-step original-operator residual with early exit, against `src/`'s restarted FGMRES, `hermite-line`, fixed restarts and no per-step residual. The block-Thomas + Sherman–Morrison apply accelerates `pr3-cm`'s dense z-local block, which `src/` never builds (Q7 carried `pr3-cm` forward as a design, not as code), so it has no landing site; and the inexact-Krylov schedule has no room, because on the shipped Cyclone deck `src/`'s shift-invert leaves 48/48 inner solves unconverged at relative residual 34 against a 1e-4 tolerance — budget-capped, not tolerance-capped, and unmoved at `shift_maxiter=2000` (96000 iterations) or in float64. A tolerance schedule cannot reduce a cost no tolerance is setting. Landing either lever means landing `pr3-cm` in `src/` first. **Done by Q28**: `pr3-cm` is in `src/`, and the two levers separate there. The exact z-block apply is **adopted** — identical iteration counts and certified residuals against a dense control, factors 2.60× smaller at (96,8,24), applies agreeing to 7.4e-16 — while the apply's *cost* advantage is size-dependent and reverses below Nl·Nm ≈ 32 (dense cheaper in 7 of 9 interleaved rounds at 32, block-Thomas in 9 of 9 at 192). The tolerance schedule is **unblocked but not landed**: with `pr3-cm` the shipped route's inner solves converge (0/96 unconverged, against 96/96 with `hermite-line`), so a tolerance does now set the cost, and the remaining prerequisite is a per-Arnoldi-step original-operator residual that `src/`'s restarted loop does not compute. The 1.15–1.22× gap this row measured is not reproduced in `src/`: there the certified shift-invert route costs 1.7–2.3× *more* than `adaptive` at the median apply cost and more at every apply cost observed, so `adaptive` stays the default. This row's own conclusion does survive: the route is **apply-bound, not iteration-bound** — `src/`'s iteration counts alone are 2.4–4.0× below the control | Q6, Q7 recorded | CPU |
| Q22 | `perf/shared-chain-forward-cost` | the forward-path cost of Q9's shared per-class transforms, measured on an idle host in #250: rk3 scan 1.026–1.035× base at both grids (failing Q9's own registered gate in 20 of 24 blocks) and the standalone RHS 1.223× at 32×32×24, against 0.88× on the RHS gradient and 0.94× on the window gradient. The change is kept for the adjoint path and the 43–48% byte reduction; this row recovers the forward cost, whose suspected source is the per-class stack concatenates (11→18 per RHS). Schedule after Q10's layout switch, which moves the same concatenates | Q9 merged, Q10 switch landed | office idle cores |
| Q23 | `fix/supplied-states-below-runtime` | Q19 follow-up — **done, #253**: one contract for the entry points below the runtime — `gkx.prepare` and the prepared object's `run`/`run_arrays` (which `PreparedSimulation.solve(initial_state=…)` reaches), the shared `integrate_nonlinear_explicit_diagnostics_state`, and the objective adjoint `nonlinear_heat_flux_window` — **project a supplied state onto the linked chain cover, never reject**: `run_arrays` is the differentiable boundary, so under `jit`/`grad` an error condition would have to read values a tracer does not have, and on a concrete array it would force a host synchronization on the route whose purpose is a reused compiled graph, while the runtime above already projects. The projection is applied to `G0` in the component builder both explicit routes share, outside the jitted scan, so the compiled scan is unchanged (linked 10680 instructions `3b52df995014727e`, periodic 9976 `ff768e7fe5cecfae`, both arms). Measured at saturated amplitude on the linked nonlinear grid: one RHS's chain rows move 2.369696e-02 relative, an 8-step run 1.060277e+00 in the final state and 3.351342e+00 in heat flux, the objective 2.26e-04 and its adjoint gradient 2.34e-04 — all exactly zero afterwards, with a supplied state's off-chain cotangent exactly zero and its chain cotangent bitwise unchanged. Free energy is *not* a discriminator here (1.0000×, against Q19's 114.9×): on a full nonlinear grid the chain cover equals the two-thirds dealias mask, so the reason to project is the bracket, not the diagnostics | Q19 merged | CPU |
| Q24 | mode-weight duplication (N3 prerequisite) | #248 found the ky weight rule written three times (four, counting `_cached_hermitian_mode_weight`) — **done, #254**: `gkx.core_ky_layout.hermitian_mode_weights` and `transport_mode_weights` are the one owner and all four call sites read from them. The recorded violation is resolved: an even grid's Nyquist row takes weight 1, not 2, because `fftfreq` stores `\|ky\| = Ny/2` exactly once and the row has no twin to fold in. Finding it needs `Ny`, which a half axis cannot supply, so `SpectralGrid` and `LinearCache` carry `ny_full`, propagated only for a complete axis and recognized by value. Measured, not assumed: 560 of 640 weight cases are bitwise against main and the 80 that move are exactly the undealiased half-axis Nyquist rows of the eight even `Ny`; the RHS/VJP gate is 58/58 and the 100-step trajectory gate 65/65 bitwise in f32 and x64, `heat_flux_t`, `Wg_t` and `Wphi_t` included. The two-sided flux convention is deliberately unchanged (whether it should represent the Nyquist row at all is the state switch's call). `tests/conftest.py` and the Q10 stage-1 driver pin `--xla_cpu_multi_thread_eigen=false`, with the reason written at the pin | Q10 partial merged | CPU |
| Q25 | `docs/methods-and-features` | README feature summary and methods documentation — **done, #256**: the README gains a *What GKX offers* section of eleven bullets, placed above Install, each naming a method or feature and linking to the page that carries its evidence; `docs/algorithms.rst` is rewritten from a thin, partly stale duplicate of `operators.rst` into **Methods and Decisions**, the entry page of the physics section, which states each decision, the measurement behind it and the boundary of that measurement, and links out rather than restating. Four figures, one committed generator (`tools/artifacts/build_methods_figures.py`), all four rendered from the evidence directories under `plan/research/scripts/` with no literals: Laguerre ladder convergence next to the certified eigenvectors' l-spectra (#238, #252), the optimized-HLO ledger of the shared chain transforms next to their idle-host A/B/A/B wall time (#243, #250), certified-eigenpair cost on the production chain (#255), and the cross-code Cyclone ky scan (#245). No new measurement and no number that is not already in the log; the two figures that pair a ledger against a clock and a certified value against its convergence state exist because the page would otherwise imply a speed-up and a converged collisionless growth rate that the evidence does not support | — (*parallel*) | none |
| Q26 | `perf/fast-accurate-defaults` | §5.1 adoption-gate rewrite, then the defaults a first run actually takes — **done, #257**. The ≥3× matvec-equivalent threshold is withdrawn and replaced by *a reproducible cost reduction with no accuracy loss is worth adopting, even at 15–30%*, with the no-accuracy-loss conditions spelt out (§5.1). Acting on it: Q21's two levers are **not** adoptable in `src/` and the reason is measured, not a threshold (see the Q21 row). One default changed, and it is an accuracy default, not a speed one: the runtime assembled its initial state in `complex64` and handed it over unwidened, and `jnp.asarray` does not promote, so a run launched with `JAX_ENABLE_X64=true` — which the shipped Cyclone deck's own header instructs — stayed float32 end to end and applied the **float32** certification gate. Same rung, same environment, against pinned `254fcc7b7`: residual 1.76e-6 against a 1.19e-4 tolerance becomes 4.04e-15 against 1e-9, with γ moving 2.5e-7 relative. The float32 *default* is unchanged and is correct: on the Cyclone deck at its own resolution it returns γ=.09309106, ω=.28203276 against the float64-certified .0930912, .2820327, seven significant figures. `adaptive` stays the eigen default: `shift_invert` fails the outer gate at residual 0.99 on that deck in both precisions, and the shift is not the obstacle (`shift_source="propagator"` picks one unaided). Recorded and not changed, for want of a measurement that says so: `KrylovConfig.power_iters=200` against `dominant_eigenpair(power_iters=40)`; `TimeConfig.method="rk2"/dt=0.1` against `ExplicitTimeConfig.method="rk4"/fixed_dt=False`; `Nl=None` falling back to 24/12, which the Cyclone deck's own comment records as ~4.5% low in γ. Host was contended throughout (1-minute load 48–173 under another user's job), so every adopted claim rests on load-independent evidence — residuals, gates, iteration counts and dtypes — and no wall time is quoted as a result. **Followed up by Q28**, which lands `pr3-cm` in `src/` and re-measures both levers there. Two corrections to this row's diagnosis: the inner residual **34** was inflated by the shifted FGMRES seeding itself with `M⁻¹b`, which under right preconditioning can only start the solve behind `x = 0` (the honest figure from zero is ~1), and the cause underneath it is that `hermite-line` does not converge at that size at all — 600 unrestarted iterations leave 0.549, where `pr3-cm` reaches the 1e-4 tolerance in 252. `adaptive` remains the default on Q28's measurement too, but the shift-invert *route* now certifies on this deck where it previously failed closed | Q7, Q21 recorded | CPU |
| Q27 | `perf/linked-gather-row-order` | N3 follow-up, the gate to adopting the half layout — **done, #259: there was nothing to fix in the gather; the byte metric was wrong**. `git diff origin/main -- src/` is empty in that PR and both ledger arms reproduce #258's committed op counts and `bytes_written` on all 24 arm/graph pairs, so the compiled modules are the ones #258 measured; what moves is the measure. #258's `bytes_written` summed every `concatenate` and `copy` **anywhere in the optimized module text**, fusion interiors included, and XLA:CPU emits an instruction written inside a fusion body as index arithmetic on the fused loop without allocating for it. Every one of the 40 half-layout copies at `_linked_fft_gather_output` and `_spectral_bracket_half_core` is a fusion interior whose operand is a fusion *parameter*: the chain gather's result is materialized once and handed in (three chain-output gathers on **both** layouts, and the module-wide `gather` count *falls* 41→33), and what XLA cloned into 27 consumer fusions is the mask multiply and its re-layout. Split into the bytes that own an output buffer, the half layout wins **all twelve graphs**: RHS −63.4%/−62.5%, rk3 −73.5%/−72.4%, rk4 −71.4%/−70.3%, against a state that is only 46.9% smaller — the fall is the Hermitian completion's `concatenate` at a fusion ROOT, 17 instructions and 33.1 MB → 4 and 2.3 MB. XLA's own buffer assignment agrees (`temp_size_in_bytes` −44 to −66%). The **`Nyc` power-of-two reading is refuted directly**: a probe that compiles `grad_z_linked_fft` alone and sweeps `Ny` is flat at `Nyc = 9, 17, 25, 32, 33` — `Ny=62` gives `Nyc=32`, a power of two on the half axis and not on the full one, and behaves exactly like 17 and 33 — and on it the half arm's scratch arena is 86–88% smaller at every `Ny`. The tool now reports `materialized_bytes`, `fused_interior_bytes` and `memory_analysis`; `bytes_written` keeps its old value so the archive stays comparable. Rejected: building the chain maps in row order (a no-op — `_state_mode_rows` is constant-folded on both ledgered routes), padding `Nyc` (no size the lowering prefers), a reshape-plus-slice (the chain classes are not contiguous). Not claimed: any wall-clock number, and `fused_interior_bytes` does rise (+146/+171% on rk3), which is more strided in-fusion reads and is the one thing left for a timing run | Q10 switch merged | CPU |
| PERF-ADJOINT | `perf/eager-adjoint-compile-cache` | the eager adjoint route `nonlinear_heat_flux_window` recompiled **13 XLA modules on every call** -- four scans and nine conds, on every objective evaluation of an optimization loop, 60-74% of its wall time -- **done, this PR**. The cause is none of the three the report proposed (linked-cache rebuild, per-call closure, non-hashable static): `ClosedJaxpr` inherits `object.__eq__`/`__hash__`, so a jaxpr compares by **identity**, and an eager `lax.scan` is dispatched on the jaxpr its body was just traced into -- a four-line scan over a Python constant reproduces it with no GKX in it (`jaxpr_identity.py`). The differentiated scan is now one module-level `jax.jit`; the host-side work (linked cache, quadrature weights, Q23 chain-cover projection, the projector's `ky` layout, now a hashable signature) stays outside it, and **every array is an argument**, which is what makes the executable serve the next geometry an optimizer proposes rather than recompile for it. Cyclone 16x16x12 Nl2/Nm4, rk3, 6-step window, `value_and_grad` wrt `tprim`, jax 0.10.2 XLA:CPU, two runs of three timed calls: compiles/call **13 -> 0** on both `ky` arms, wall/call 4.92-5.36 s -> 0.139-0.141 s (two-sided, 37x) and 7.04-7.36 s -> 0.112-0.116 s (half, 63x); the cold call also falls, 11.0 -> 7.3 s and 12.4 -> 6.8 s. **Identity**: bitwise in 20 of 26 gate configurations -- every `checkpoint=True` case except rk2/x64 -- over rk2/rk3/rk4, both `ky` layouts, a window tail, the uncompressed FFT path, exact Laguerre quadrature and an EM KBM deck, in f32 and x64. The six that move are 1-11 f32 ulps and 1-3 double ulps, five of them `checkpoint=False`, against the window's own measured f32 roundoff of 48 ulps (Q14). The residual is structural: a reusable graph must take the grid and the quadrature weights as operands, and on the eager route their products were folded on the host before the body was traced (forward scan body 716 -> 719 equations, 831 -> 954 primitives counting nested jaxprs). Dividing by the window tail inside the graph was measured and rejected (one f32 ulp on three configurations); a content-keyed compile cache over the eager dispatch, which would be bitwise by construction, was rejected as reimplementing `jax.jit`'s cache on JAX internals for a few ulps. **Side effect for Q10/Q27**: the half `ky` arm looks 1.39x slower per call on `main` and is 1.22x *faster* once the compile is cached, so that A/B was measuring compile time | Q23 merged | CPU |
| Q28 | `perf/pr3cm-preconditioner` | §5.1 L4 in `src/`, then Q21's two levers re-measured there under the rewritten gate — **done, this PR**. `pr3-cm` is now code (`gkx/solvers_linear_precond_pr3.py`): three Peaceman–Rachford sweeps of the shipped Hermite line solve (streaming, hypercollisions and the z-mean drift diagonal) against an exact z-local drift/mirror/φ block, with Q7's scalar rule reproducing **α = −8.82076** at (96,8,24) against Q7's −8.81. **Q26's budget cap has three causes, not one**: the shifted FGMRES seeded itself with `M⁻¹b`, which under right preconditioning can only start the solve behind `x = 0` and is what made the reported inner residual **34** instead of ~1 (25.4/20.7/3.53 for hermite-line/field-corrected/damping on the first RHS); the real cause is that **`hermite-line` does not converge at this size** (600 unrestarted iterations leave 0.549, where `pr3-cm` reaches the 1e-4 tolerance in 252); and `shift_restart=20` stalls even `pr3-cm` (300 iterations as GMRES(20)×15 leave 0.177). **Structure measured at every build, not assumed**: off-tridiagonal entry exactly 0, rank-one ratio 6.0e-15, z-locality defect 1.6e-16; a collision operator coupling ℓ beyond ±1 falls back to the dense inverse and a grid carrying zonal (ky=0, kx>0) rows, whose adiabatic ⟨φ⟩ sums over z, is **refused**. **Lever 2 (exact block-Thomas + Sherman–Morrison z-block) adopted**: identical iteration counts and certified residuals against the dense control (13996 and 3.25e-07 in all four arms; applies agreeing to 7.4e-16), factors 2.60× smaller at (96,8,24) — Q21's 2.59× — while the apply changes sign with block size, the dense inverse cheaper in 7 of 9 interleaved rounds at Nl·Nm=32 and block-Thomas cheaper in **9 of 9** at 192, so `"auto"` is not made unconditional. **Lever 1 (inexact-Krylov schedule) unblocked, not landed**: with `pr3-cm` the inner solves converge (**0/96** unconverged against hermite-line's 96/96), so a tolerance now sets the cost; what is missing is a per-Arnoldi-step original-operator residual to drive it. **Gate verdict: `adaptive` stays the default** — certified shift-invert costs 1.68× more matvec-equivalents at (96,4,8) (≈57000 against 33916) and 2.27× more at (96,8,24) (≈220000 against 96700) at the median `c_P`, and **more at every `c_P` observed** in eighteen interleaved rounds, so it is a cost increase rather than a reduction. The gap is entirely the apply: the iteration counts alone (13996, 24188) are *below* the control, so a free apply would make it 2.4× and 4.0× cheaper — the route is apply-bound, not iteration-bound, which is Q21's conclusion reproduced in `src/`. `c_P` itself had to be re-measured by interleaving: the arm files' values are wrong (four readings of one dense apply gave 0.96, 6.04, 18.08, 19.30) because they time the matvec and the apply twenty minutes apart on a host at load 7–70. What is adopted is the route: on the shipped deck shift-invert went from rejected at outer residual 0.995/0.998 to **certified at 3.25e-07 and 1.67e-07**, eight significant figures of the control's γ. Not reached: the production chain (96,16,48), and no wall-clock claim — the host carried four lanes at load 7–70 throughout | Q7, Q21, Q26 recorded | CPU |
...
|---|---|---|
| Research-grade release 2.4.0 (2.1.0, 2.2.0 and 2.3.0 were cut as intermediate releases on 2026-09-15, 2026-09-20 and 2026-09-21; these exits were not required for any of them) | #192 repaired; every README/docs number ledger-gated; ES atlas and nonlinear validation closed; EM0–EM3 closed, EM4 linear closed; statistics protocol; citation and DOI | 0–3, 7 |
...
2. `CITATION.cff` (added). Mint a Zenodo DOI at 2.4.0 and add the badge.
...
**Updated 2026-09-18 by the Q16 eigen runs** ([log](plan/log.md)): step (ii) is
done at Nl 24/32/48 and answers itself. The certified adaptive eigenpairs
reproduce Q8's ladders (every settled fit to ≤0.01%, the two unsettled
collisionless fits differing by 0.57% and 7.21%), so the non-monotone small-ν
ladders are a property of the dominant eigenvalue, and the eigenvectors show
they are **one branch, not a branch change**: the φ(z) overlap is ≥ 0.991 on all
30 pairs and ≥ 0.9974 across the Nl32 dip, with ω inside a 2.2% band throughout.
The Laguerre spectrum of the eigenvector separates the ladders exactly as the
convergence does: ν=1e-2, the only Nl-converged ladder, is the only one with a
monotone decaying spectrum (no interior local maximum, 5.30 and 7.28 decades
peak to cutoff at Nl 32/48), while every unconverged ladder carries a cutoff
pile-up whose last maximum sits at 0.79–0.85 of Nl at every rung, so raising Nl
relocates it rather than resolving it; collisionlessly the interior-maximum
count grows with Nl (2, 3, 4) and the upper quarter holds 1.33/1.02/0.54% per
index at Nl 24/32/48, reproducing from the eigenvector the 8% stationary plateau
the 2026-09-13 review measured on the GX pair. Working rule for this deck, not a
general tolerance: the ladder converges once the spectrum has no interior
maximum and the upper quarter holds ≲2e-3. No collisionless Laguerre-converged
mode exists at this ky up to Nl48; Nl64 was not reached (45-min cap). Step (ii)
therefore hands the question to (iii)/(iv) — a declared Laguerre sink or a
declared regularization — rather than to another rung.
...
recorded; EM5 linear/QL derivative rows passing. Required for 2.4.0.
...
adopting under the §5.1 adoption gate below; L5 recycled inner solves (SOLVAX `gcrot` harmonic, i.e. GCRO-DR)
...
**The L4/L5 adoption gate, rewritten 2026-09-19.** This section used to adopt a
solver candidate only at **≥3× fewer matvec-equivalents** to a certified pair. That
threshold is withdrawn. It was a round number rather than a measured one, and it
rejected changes that cost nothing to keep: Q7 and Q21 both ran candidates that were
cheaper, exact and certified, and recorded them as failures only because they were
not cheaper by a factor of three. The gate is now:

> **A reproducible cost reduction with no accuracy loss is worth adopting, even at
> 15–30%.**

The conditions are the part that is not negotiable. *No accuracy loss* means the
certification is applied exactly as before, the returned pair still passes the
original-operator residual gate, and no shipped validation gate moves; a candidate
that buys speed by relaxing a tolerance, by changing the returned pair beyond the
gate, or by moving a validated number is rejected however fast it is.
*Reproducible* means the reduction is measured on the same host in the same session,
setup included, and repeats — and where host load makes a clean timing impossible,
it is carried by load-independent evidence (matvec-equivalents, iteration counts,
HLO bytes, resident memory) rather than by a wall time nobody else can reproduce. A
memory reduction or an exactness property established at an unchanged iteration
count is a cost reduction in its own right. What this gate does **not** do is make a
candidate the default: a route that is cheaper than the runtime default only at
sizes or settings a first run never reaches is adopted as an improvement to that
route, not as the route a user gets without asking for it.
...
**Inner-solve cost (L5, 2026-09-19; [log](plan/log.md)).** The L5 levers close most of
Q7's gap and still fail the gate. On one host in one session at the production chain, the
`adaptive` control certifies in 195196 matvec-equivalents (310 s, twice, to the
application), Q7's configuration reproduces at 443930 (748 s, 13055 inner iterations), and
the best arm reaches 160254–169875 (261 s, 6961 inner iterations) — **1.15–1.22× fewer
than `adaptive`, against a gate of ≥3× fewer**, so `adaptive` stays the runtime default
and no `src/` change follows. Two levers give the 2.8×. Q7's own candidate, an inner
tolerance ∝ the outer residual (Freitag–Spence inexact inverse iteration), does not
certify at all on a shift-invert *Arnoldi*: loose early solves perturb the basis and the
outer residual stalls at 1.5e-3. The inexact-Krylov rule (Simoncini–Szyld), which relaxes
the *late* steps instead — `tol_j ∝ target/res_{j-1}` — reproduces the fixed-tolerance
outer residual sequence step for step at 1.67× the speed (inner iterations 13055 → 6961).
An exact block-Thomas + Sherman–Morrison solve of the z-local block, which is exactly
ℓ-tridiagonal plus a rank-one field part and so leaves every iteration count unchanged,
gives a further 1.66× and cuts the factors 5.26× (864 → 164 MB); its apply is only 1.34×
faster in isolation, not the 4× its flop count implies. Rejected: the tuned preconditioner
P + (A−P)XXᴴ (1.04× alone, negative once the schedule is on) and harmonic Rayleigh–Ritz
without inner solves, which stalls at residual 0.4–0.5 and needs a basis ≳90% of n to
converge at all. The binding constraint is now the preconditioner apply, not the iteration
count: with a free apply the same 6961 iterations would cost 7119 matvec-equivalents, 27×
under the gate, so L6's shift placement and a device apply are the remaining levers.

**Q21's levers against the rewritten gate (Q26, 2026-09-19; [log](plan/log.md)).** The
gate above was rewritten so that a 15–30% reproducible saving is adoptable, and Q21's
two levers were then re-examined for adoption. Neither is adoptable in `src/`, and the
reason is not the threshold: **the shift-invert Q21 measured is not the shift-invert
GKX ships.** Q21's harness runs SOLVAX `gcrot` with subspace recycling, the `pr3-cm`
structured preconditioner, an unrestarted Arnoldi and a per-step original-operator
residual with early exit; `src/` runs restarted FGMRES with no recycling, the
`hermite-line`/`field-corrected` preconditioners, a fixed `restarts × krylov_dim` loop
and no per-step residual at all. The block-Thomas + Sherman–Morrison apply is an exact
solve of `pr3-cm`'s dense z-local block, and `src/` never builds that block — Q7 adopted
`pr3-cm` as *the L4 design to carry forward*, not as code — so it has no landing site.
The inexact-Krylov schedule has no room: on the shipped Cyclone deck at (Nz,Nl,Nm) =
(96,4,8), `KrylovConfig(method="shift_invert")` leaves **48 of 48** inner solves
unconverged at maximum relative residual 34 against its 1e-4 inner tolerance, every
solve at the 60-iteration budget cap, and the outer pair is rejected at residual 0.99.
Raising `shift_maxiter` 50 → 2000 (2880 → 96000 inner iterations) leaves the residual at
35, and float64 gives 48/48 unconverged at 34 with outer residual 0.987, so it is
neither a budget shortfall nor a precision effect. A tolerance schedule cannot reduce a
cost that no tolerance is setting. Both levers stay available and both become landable
together, behind one prerequisite: `pr3-cm` in `src/`, which is a separate adoption with
its own certification evidence, proposed as the next row.
...
actually compiles, #239 and #249, and `--route eager-scan` for the placement
#249 retired) plus A/B/A/B timings on an idle pinned checkout,
...
**#249 closed the other way** (Q18): the runtime entry point now runs the
...
3. Release 2.4.0 only when: Phase 0–3 exits met (EM4 nonlinear excepted),
...
- [ ] 7 docs generated from the ledger; examples regenerate; paper 1 bundle; 2.4.0 reviewed by the maintainer.
