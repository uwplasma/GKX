...
- **Scope is this repository.** All work here lands in GKX. VMEX, ESSOS, and
  SOLVAX belong to other developers: read them for reference and depend on
  them, but do not reorganize or refactor them from this plan. The single
  exception is a change one of them must make for a specific GKX capability --
  then make that one targeted change in that repository, and record here which
  GKX capability required it.
...
| GOV-1 | P0 | landed | PR #83 removes plan from main; PR #82 stays open | plan absent from main, branch recoverable |
| GOV-2 | P0 | landed | publish the size cutover: the candidate is rebuilt to public `main` and verified (complete `.git` 9,148,556 B, source tree byte-identical, zero attribution hits, 124 release gates + 1,187 unit tests green in a fresh clone). Branch heads RESOLVED 2026-08-23: 84 merged heads deleted after bundle verification, leaving `main`, the roadmap head, and two closed-unmerged heads, so the cutover is a `main`-plus-tags force-push rather than an 84-head replay. Recovery bundle re-frozen (`GKX-pre-rewrite-2026-08-23.bundle`, 286,768,136 B, complete history, 88 heads + 28 tags). Battery, bundle and size/integrity gates all PASSED 2026-08-23 (complete `.git` 9,410,553 B; 1,853 tests green; zero attribution hits). Tag messages scrubbed to `GKX vX.Y`; commit messages deliberately left historical per owner decision, so no re-SHA. Remaining: rebase the one roadmap head, publish the bundle + SHA-256 + ref map + re-clone instructions, then force-push `main` and tags and restore protection | a real network clone measures under 10 MB (owner's outer bound 20 MB); bundle + ref map published first; protection restored after |
| SLIM-4 | P0 | open | **organize the source code, minimizing both files and folders.** Today: 41 directories and 207 files under `src/gkx`, plus 83 under `docs`, 49 under `tests`, 13 under `tools` (186 directories total). For scale reference only (no work lands in those repositories): ESSOS ships 17 flat modules with no subpackages; the VMEX solver core is 46 files. Target the manifest's six domains: collapse the 20-subpackage tree, flatten single-owner leaf directories, and merge modules that exist only to hold one function. Sequenced with SLIM-2 (Deps layer) and SLIM-3 (campaign governance eviction), which together remove roughly 15k lines and are what make the folder collapse safe | directory AND file counts recorded per tree in the architecture manifest and falling every PR; public import paths preserved or aliased; no module moved merely to lower a count; JIT/AD semantics and CPU/GPU numerics unchanged |
| DOC-1 | P1 | open | reorganize `README.md` using the VMEX README only as a read-only style reference (no work lands in that repository) -- **length is not the problem, organization is**. That README's pattern: badges, then ONE sentence saying what the code is, a hero figure, an install block whose commands actually run (`pip install vmex; vmex --doctor; vmex --test`), then task-titled sections ("Solve and inspect an equilibrium", "Hot restart"), and -- the key property -- every quantitative claim carries a reproduction command, pinned revisions of everything compared, and explicit scoping of what the comparison does and does not assert. GKX's 615 lines may stay 615; each section must earn its place, state its evidence, and read in short declarative sentences | a first-time reader reaches a working `gkx wout_XXX.nc` inside one screen; every number in the README names the command and revision that produced it; no claim without evidence |
| REL-1 | P0 | landed | cut **GKX v1.8.0** after the size cutover so the tag is born on rewritten history. The 22 historical GitHub releases were retitled from `SPECTRAX-GK vX.Y` to `GKX vX.Y` and their bodies scrubbed (old package/repo names rewritten to `gkx`/`GKX`) on 2026-08-23; zero `spectrax` strings remain in any release title or body, and none in tracked source | release readiness gate green; tag on post-rewrite `main`; release notes carry the session's validated changes |
...
| RUN-1 | P0 | landed | PR #84 exact horizon and 128-step checks | demonstrated on the supplied QA artifact; all 41 checks green |
| SAT-1 | P0 | landed | PR #91 fixed-horizon replay, Q/Wphi/Wg gates, and output locks | QHS is seed-sensitive; QI Ny160 makes no causal stop; first matched QA pair passes and replicated queues are active |
| GEO-1 | P0 | landed | PR #86 physical VMEC tube coordinates | NetCDF round trip + Cartesian coordinate test; all 41 checks green |
| RES-1 | P0 | landed | PR #87 spectrum-tail warnings + QA/QHS/QI Ny scan | QI Ny160 clears tails but shifts 4.67% from Ny128; QA Ny192 pair and later QI Ny192 are required |
| VAL-0 | P0 | landed | PR #89 per-trace QA audit; PR #90 promotion evidence contract | all 41 checks green on each; promotion remains false |
| UX-1 | P1 | landed | PR #85 startup glossary | definitions pass; fixed horizon retained until SAT-1 passes |
| MOV-1 | P1 | landed | PR #96 physical cuts + PR #97 production-state continuation | rendering passes; hash-bind source state and PR #91 identity before evidence use |
| VAL-1 | P1 | active | QA, QHS, and QI fixed-horizon campaign | paired CI + resolution + zonal gates |
| AD-1 | P1 | landed | PR #100 narrows the finite-window adjoint claim | repeat source-pinned AD/FD knee, CPU/GPU, and optimized-equilibrium gates |
| SLIM-1 | P1 | landed | PRs #88/#95/#102--#105/#107--#114 remove redundant renders/grids/traces/policies/setup/formulas; latest pre-record rehearsal `.git` is below 9.75 MB | freeze the complete 111-ref map, publish recovery records, then real network-clone gate |
| OUT-1 | P1 | landed | PR #94 fails closed on rejected plot windows, including the one-page summary | supplied QA replot, focused tests, and all 41 CI checks pass |
