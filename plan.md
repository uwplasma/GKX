...
| SLIM-TOOLS | #278 | `slim/tools-benchmarks-1` @ `33d2dc0fc` | Tranche 1: 16 unused files (3,757 lines) deleted, `scripts/checks/*` moved to `scripts/checks/` behind `scripts/check.py`; tools Python 96 → 78 files, 78,901 → 63,832 lines; full map in `plan/research/2026-09-22-slim-tools/MAP.md` | CI, then tranche 2 from MAP.md |
...
1. #278 SLIM-TOOLS tranche 1. Already merged with main at `ac65b570d`, and its repo-hygiene step passes locally. It moves `scripts/checks/*` behind `scripts/check.py`. Every later PR that calls `scripts/checks/...` must switch to `python scripts/check.py <subcommand>`.
...
generator = "scripts/comparison/build_gx_parity_matrix.py --case cyclone_salpha_itg"
...
4. **HSX: fix the provenance, do not delete the row.** *Corrected 2026-09-08;
   ...
   `scripts/comparison/compare_gx_imported_linear.py` against
...
2. **GX goldens at GX tolerance** for the six cases. Either the Cyclone 5–7%
   ...
   `scripts/comparison/build_gx_parity_matrix.py`. Rows: `L-lin-*`.
...
1. **Adapter** `scripts/comparison/landreman_dataset.py`: read tube i from the
...
`scripts.profiling.profile_runtime_kernels._runtime_memory_summary`; the profiler
...
Keep the README within 270 lines. Order: pitch → figure → install →
...
| Nonlinear EM AD at scale | EM5 derivatives pass; a representative pilot establishes peak forward/VJP memory, wall time and communication within the available allocation |
