...
@pytest.mark.parametrize(
    ("ny", "ky_index", "kx_index"),
    [(8, 0, 1), (8, 2, 1), (8, 2, 0), (8, 4, 3), (8, 6, 1), (7, 3, 1), (7, 5, 2)],
)
def test_fixed_mode_stage_projection_is_the_full_projection_on_complete_states(
    ny: int, ky_index: int, kx_index: int
) -> None:
    """ky=0, paired, Nyquist and negative-row fixed modes, even and odd Ny."""

    ky = np.fft.fftfreq(ny)
    key_re, key_im, key_fixed = jax.random.split(jax.random.PRNGKey(ny + ky_index), 3)
    shape = (2, ny, 4, 3)
    trial = jax.random.normal(key_re, shape) + 1j * jax.random.normal(key_im, shape)
    fixed = jax.random.normal(key_fixed, shape).astype(jnp.complex64) * (1.0 - 2.0j)
    hermitian = _make_hermitian_projector(ky, nx=4)
    complete = hermitian(trial.astype(jnp.complex64))
    projector = _make_nonlinear_state_projector(
        fixed,
        ky_vals=ky,
        nx=4,
        compressed_real_fft=True,
        fixed_mode_ky_index=ky_index,
        fixed_mode_kx_index=kx_index,
    )
    stage = nonlinear_projection._stage_projection(projector)

    assert np.array_equal(np.asarray(stage(complete)), np.asarray(projector(complete)))
    assert nonlinear_projection._stage_projection(hermitian)(complete) is complete
    plain = lambda state: state  # noqa: E731
    assert nonlinear_projection._stage_projection(plain) is plain


@pytest.mark.parametrize("method", ["rk3", "rk4"])
def test_hermitian_completion_once_per_step_is_bitwise_the_per_stage_route(
    method: str,
) -> None:
    """The RHS returns completed derivatives, so stage completions are exact no-ops.

    A plain callable keeps the per-stage completion; the cached projector
    completes once per step. The trajectories must agree bit for bit.
    """

    cfg = CycloneBaseCase(grid=GridConfig(Nx=8, Ny=8, Nz=8, Lx=62.8, Ly=62.8))
    grid = build_spectral_grid(cfg.grid)
    geom = SAlphaGeometry.from_config(cfg.geometry)
    params = LinearParams()
    cache = build_linear_cache(grid, geom, params, Nl=2, Nm=2)
    projector = _make_hermitian_projector(np.asarray(grid.ky), nx=grid.kx.size)
    shape = (2, 2, cfg.grid.Ny, cfg.grid.Nx, cfg.grid.Nz)
    key_re, key_im = jax.random.split(jax.random.PRNGKey(3))
    G0 = 0.1 * (
        jax.random.normal(key_re, shape) + 1j * jax.random.normal(key_im, shape)
    )
    ...
    def run(project_state):
        return nonlinear_state_integration_mod.integrate_nonlinear_scan(
            ...
            rhs_static_args=(TermConfig(nonlinear=1.0), True, "grid", None),
        )

    once_state, once_fields = run(projector)
    staged_state, staged_fields = run(lambda state: projector(state))

    assert np.isfinite(np.asarray(once_state)).all()
    assert np.array_equal(np.asarray(once_state), np.asarray(staged_state))
    assert np.array_equal(np.asarray(once_fields.phi), np.asarray(staged_fields.phi))
