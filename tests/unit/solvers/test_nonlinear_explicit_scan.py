...
def test_discrete_nilsas_recovers_unstable_fixed_point_sensitivity() -> None:
    """NILSAS must remove an exponentially growing initial-value tangent."""

    amplification = jnp.asarray(2.0, dtype=jnp.float64)
    parameters = jnp.asarray([0.3], dtype=jnp.float64)

    def step(state: jnp.ndarray, values: jnp.ndarray) -> jnp.ndarray:
        return amplification * state + values

    def objective(state: jnp.ndarray, _values: jnp.ndarray) -> jnp.ndarray:
        return state[0]

    fixed_point = -parameters / (amplification - 1.0)
    result = discrete_nilsas(
        ...
        segment_count=48,
        steps_per_segment=3,
        homogeneous_adjoint_count=1,
    )

    np.testing.assert_allclose(result.value, fixed_point[0], rtol=5.0e-7)
    np.testing.assert_allclose(result.gradient, [-1.0], rtol=3.0e-2, atol=3.0e-2)
    assert float(result.constraint_residual) < 1.0e-6


def test_discrete_lyapunov_exponents_recover_diagonal_map_spectrum() -> None:
    diagonal = jnp.asarray([1.8, 1.2, 0.7], dtype=jnp.float64)

    def step(state: jnp.ndarray, _values: jnp.ndarray) -> jnp.ndarray:
        return diagonal * state

    result = discrete_lyapunov_exponents(
        step,
        jnp.asarray([0.2, -0.1, 0.3], dtype=jnp.float64),
    ...
    expected = np.log(np.asarray(diagonal)) / 0.25

    np.testing.assert_allclose(
        np.sort(np.asarray(result.exponents)), np.sort(expected), rtol=2.0e-5
    )
    orthogonality_tolerance = 1.0e-10 if jax.config.x64_enabled else 1.0e-5
    assert float(result.orthogonality_residual) < orthogonality_tolerance


def test_windowed_discrete_adjoint_matches_plain_scan_and_finite_difference() -> None:
    """The production-window helper must preserve a non-square discrete map."""

    def step(state: jnp.ndarray, values: jnp.ndarray) -> jnp.ndarray:
        return jnp.tanh(state + values[0])

    def objective(state: jnp.ndarray, values: jnp.ndarray) -> jnp.ndarray:
        return jnp.sum(state * state) + 0.1 * values[0]

    state = jnp.asarray([0.2, -0.4], dtype=jnp.float64)
    parameters = jnp.asarray([0.03], dtype=jnp.float64)
    blocked = discrete_window_value_and_grad(
        step,
        objective,
        state,
        parameters,
    ...
    plain = discrete_window_value_and_grad(
        step,
        objective,
        state,
        parameters,
    ...
    increment = jnp.asarray([1.0e-5 if x64 else 1.0e-3], dtype=parameters.dtype)
    plus = discrete_window_value_and_grad(
        step,
        objective,
        state,
    ...
    minus = discrete_window_value_and_grad(
        step,
        objective,
        state,
        parameters - increment,
    ...
    centered_fd = (plus - minus) / (2.0 * increment[0])

    scan_rtol = 1.0e-12 if x64 else 2.0e-7
    fd_rtol = 2.0e-7 if x64 else 5.0e-4
    np.testing.assert_allclose(blocked[0], plain[0], rtol=scan_rtol)
    np.testing.assert_allclose(blocked[1], plain[1], rtol=scan_rtol)
    np.testing.assert_allclose(blocked[1][0], centered_fd, rtol=fd_rtol)


def test_discrete_nilsas_returns_all_parameter_derivatives_from_one_adjoint() -> None:
    """One shadowing adjoint should recover stable and unstable sensitivities."""

    matrix = jnp.diag(jnp.asarray([1.7, 0.6], dtype=jnp.float64))
    weights = jnp.asarray([1.0, 0.4], dtype=jnp.float64)
    parameters = jnp.asarray([0.2, -0.3], dtype=jnp.float64)

    def step(state: jnp.ndarray, values: jnp.ndarray) -> jnp.ndarray:
        return matrix @ state + values

    def objective(state: jnp.ndarray, values: jnp.ndarray) -> jnp.ndarray:
        return weights @ state + 0.2 * values[0]

    fixed_point = jnp.linalg.solve(jnp.eye(2) - matrix, parameters)
    result = discrete_nilsas(
        step,
        objective,
        fixed_point,
    ...
    expected = weights @ jnp.linalg.inv(jnp.eye(2) - matrix)
    expected = expected + jnp.asarray([0.2, 0.0])

    np.testing.assert_allclose(result.gradient, expected, rtol=5.0e-2, atol=5.0e-2)
    assert np.all(np.isfinite(np.asarray(result.coefficients)))
    assert np.isfinite(float(result.kkt_condition_number))


def test_matrix_free_multiple_shooting_recovers_unstable_sensitivity() -> None:
    """MSS must select the bounded tangent without assembling a Jacobian."""

    amplification = jnp.asarray(1.8, dtype=jnp.float64)
    parameters = jnp.asarray([0.3], dtype=jnp.float64)
    ...
    expected = -1.0 / (amplification - 1.0)
    np.testing.assert_allclose(result.value, fixed_point[0], rtol=5.0e-7)
    np.testing.assert_allclose(result.gradient[0], expected, rtol=3.0e-2)
    assert float(result.normal_residual) < 1.0e-6


def test_matrix_free_multiple_shooting_returns_all_parameter_derivatives() -> None:
    matrix = jnp.diag(jnp.asarray([1.7, 0.6], dtype=jnp.float64))
    weights = jnp.asarray([1.0, 0.4], dtype=jnp.float64)
    parameters = jnp.asarray([0.2, -0.3], dtype=jnp.float64)
    ...
    result = discrete_multiple_shooting_shadowing(
        ...
        regularization=1.0e-10,
        cg_tolerance=1.0e-10,
        cg_max_iterations=384,
    )
    expected = weights @ jnp.linalg.inv(jnp.eye(2) - matrix)
    expected = expected + jnp.asarray([0.2, 0.0])

    np.testing.assert_allclose(result.gradient, expected, rtol=5.0e-2, atol=5.0e-2)
    assert int(result.cg_iterations) <= 384
