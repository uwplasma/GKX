        ...
        "controls": [
            {
                ...
                "nonlinear_campaign_command_after_vmec_runs": (
                    f"python scripts/campaigns/write_nonlinear_turbulence_gradient_campaign.py --out-dir {nested_manifest.parent}"
                ),
            }
        ],
...
def test_overdetermined_status_cli_writes_json_and_fail_code(tmp_path: Path) -> None:
    ...
    assert (
        mod.main(["overdetermined-gradient", str(manifest), "--out-json", str(out)])
        == 0
    )
    assert (
        mod.main(
            [
                "overdetermined-gradient",
                str(manifest),
                "--out-json",
                str(out),
                "--fail-on-blocked",
