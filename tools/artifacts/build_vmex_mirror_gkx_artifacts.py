...
def _case():
    ...
    return inputs, setup, geometry, mapping, np.asarray(objectives)
...
def build(output_dir: Path, *, movie: bool = True) -> dict:
    inputs, setup, geometry, mapping, objective = _case()
    ...
    axb.plot(theta / np.pi, bmag, lw=2.2, label=r"$B/B_{ref}$")
    axb.plot(theta / np.pi, bgrad, lw=1.7, label="bgrad")
    axb.axhline(0.0, color="0.75", lw=0.8)
    axb.set(xlabel=r"equal-arc $z/\pi$", title="mirror force on the GKX grid")
    axb.legend(frameon=False)

    axm = figure.add_subplot(1, 3, 3)
    axm.plot(theta / np.pi, np.asarray(geometry.gds2_profile), label="gds2")
    axm.plot(theta / np.pi, np.asarray(geometry.gds21_profile), label="gds21")
    axm.plot(theta / np.pi, np.asarray(geometry.gds22_profile), label="gds22")
    axm.set(xlabel=r"equal-arc $z/\pi$", title="perpendicular metric")
    ...
    axm.text(
        ...
        "\n"
        rf"$Q_{{QL}}={objective[-1]:.4f}$, $R_m={bmag.max()/bmag.min():.3f}$",
    ...
    if movie:
        ...
        movie_profile.plot(theta / np.pi, bmag, color="#31688e", lw=2.2)
        marker = movie_profile.axvline(theta[0] / np.pi, color="#d1495b", lw=1.5)
        movie_profile.set(
            xlabel=r"equal-arc $z/\pi$",
            ylabel=r"$B/B_{ref}$",
