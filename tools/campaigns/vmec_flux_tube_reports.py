...
def _vmec_flux_tube_mapping_fn(
    *,
    ...
    def mapping_fn(x: jnp.ndarray) -> dict[str, Any]:
        ...
        # ported (gkx.geometry.vmec_tensor_mapping).
...
def vmex_flux_tube_sensitivity_report(  # pragma: no cover
    *,
    ...
    the GKX ``FluxTubeGeometryData`` mapping, and differentiates
...
def _imported_vmec_eik_geometry(*, case_name: str, request: SimpleNamespace) -> Any:
    ...
    with tempfile.TemporaryDirectory(prefix="gkx_vmec_eik_parity_") as tmp:
