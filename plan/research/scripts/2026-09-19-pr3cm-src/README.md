...
`costmodel.py`
: the preconditioner apply cost `c_P`, and the only number in this directory
  that `measure.py` gets wrong. `measure.py` times the matvec at the start of an
  arm and the apply at the end, twenty minutes apart on this host; under a load
  that moved between 7 and 70 that is not a ratio, and four readings of the same
  dense apply returned 0.96, 6.04, 18.08 and 19.30. This script builds every
  candidate up front and then runs `--blocks` rounds that time the matvec and
  each apply back to back, rotating the order. **Use its `c_precond` and not the
  arm files'.**
...
: one fresh, cold, single-threaded process per arm, run serially, with the host
  ...
  `run_production.sh` and Q21's `costmodel.py`
: the preconditioner apply cost `c_P`, and the only number in this directory
  that `measure.py` gets wrong. `measure.py` times the matvec at the start of an
  arm and the apply at the end, twenty minutes apart on this host; under a load
  that moved between 7 and 70 that is not a ratio, and four readings of the same
...
| `cost/` | the interleaved `c_P` rounds at both rungs (`costmodel.py`) |
...
`SCRIPT=diagnose.py` selects the diagnostic and `SCRIPT=costmodel.py` the
apply-cost rounds.
...
quantities -- inner-iteration counts, factor bytes, the certified residual and
the returned pair -- are what the verdict rests on. Matvec-equivalents depend on
`c_P`, so every conclusion drawn from them is checked at the cheapest `c_P`
observed as well as at the median.
