...
"""
# ruff: noqa: E402  (sys.path set up before importing gkx)
...
def unbarriered_advance(G, dG, dt_local, *, method, rhs_fn, project_state, state_dtype):
    G_new = sne._explicit_stage_update(
        G, dG, dt_local, method=method, rhs_fn=rhs_fn, project_state=project_state
    )
...
def stage_only_advance(G, dG, dt_local, *, method, rhs_fn, project_state, state_dtype):
    ...
    G_new = sne._explicit_stage_update(
        G, dG, dt_local, method=method, rhs_fn=rhs_fn, project_state=projection
    )
    return jnp.asarray(project_state(G_new), dtype=state_dtype)
...
def compile_variant(name):
    ...
    sni.advance_explicit_nonlinear_state = {
        "main": unbarriered_advance,
        "mainblock": unbarriered_advance,
        "stage": stage_only_advance,
    }.get(name, original_advance)
    budget = None
...
results = {
    n: {"compile_s": arms[n][2], "temp_bytes": arms[n][3], "vjp": [], "fwd": []}
    for n in names
}
for n in names:  # warm
...
for n in names:
    ...
    r["value_rel_to_" + ref] = abs(r["value"] - results[ref]["value"]) / abs(
        results[ref]["value"]
    )
    r["grad_rel_to_" + ref] = abs(r["grad"] - results[ref]["grad"]) / abs(
        results[ref]["grad"]
    )
    print(
        f"{args.ky} {n:8s} vjp {r['vjp_median']:8.3f}s fwd {r['fwd_median']:7.3f}s "
        f"temp {r['temp_bytes'] / 2**20:8.1f}MiB compile {r['compile_s']:6.1f}s "
        f"dval {r['value_rel_to_' + ref]:.1e} dgrad {r['grad_rel_to_' + ref]:.1e}",
        flush=True,
    )
record = {
    "jax": jax.__version__,
