...
def bench(name, fn, *a):
    ...
    print(
        f"{args.ky} {name:24s} median {1e3 * statistics.median(ts):8.2f} ms  min {1e3 * min(ts):8.2f} ms",
        flush=True,
    )
...
def rhs_full(G, c):
    return sni.nonlinear_rhs_cached(
        G,
        c,
        params,
        terms,
...
proj_barrier = (
    (lambda G: jax.lax.optimization_barrier(G)) if args.ky == "half" else proj
)
...
def step_b(G, c):
    ...
    return advance_explicit_nonlinear_state(
        ...
        method="rk3",
        rhs_fn=rhs,
        project_state=proj_barrier,
        state_dtype=G.dtype,
    )


bench("rk3_step_barrier", step_b, g0, cache)
a = jax.jit(step)(g0, cache)
b = jax.jit(step_b)(g0, cache)
print(
    "bitwise",
    bool(jnp.all(a == b)),
    float(jnp.max(jnp.abs(a - b)) / jnp.max(jnp.abs(a))),
)


def step_c(G, c):
    def rhs(s):
        d, f = sni.nonlinear_rhs_cached(
            s,
            c,
            params,
            terms,
