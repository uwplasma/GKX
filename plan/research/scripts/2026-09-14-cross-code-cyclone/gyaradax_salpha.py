...
record = {
    "ky_gx": args.ky_gx, "krhomax": krho, "rung": args.rung, **r, "naverage": args.naverage,
    "t_max": nwin * r["dt"] * args.naverage, "host": platform.node(), "jax": jax.__version__,
    "devices": [str(d) for d in jax.devices()], "gyaradax_file": gyaradax.__file__,
    "x64": bool(jax.config.jax_enable_x64), "cuda_visible_devices": os.environ.get("CUDA_VISIBLE_DEVICES"),
}

geom = compute_geometry(
    q=1.4, shat=0.8, eps=0.18, ns=r["ns"], nvpar=r["nvpar"], nmu=r["nmu"], vpar_max=3.0,
    nkx=1, nky=1, nperiod=r["nperiod"], kxmax=krho, signB=1.0, Rref=1.0, krhomax=krho,
    geom_type="s-alpha",
)
params = GKParams(
    dt=r["dt"], naverage=args.naverage, non_linear=False, adaptive_dt=False, adiabatic_electrons=True,
    disp_par=1.0, disp_vp=0.0, disp_x=0.0, disp_y=0.0, finit="cosine2", amp_init=1e-4,
    mas=1.0, signz=1.0, tmp=1.0, de=1.0, vthrat=1.0, rlt=2.49 * R_OVER_A, rln=0.8 * R_OVER_A,
    dgrid=1.0, tgrid=1.0, sgr_dist=float(geom["sgr_dist"]), dvp=float(geom["dvp"]),
    kxmax=krho, kymax=krho, norm_eps=1e-14, drive_scale=1.0, idisp=2, cfl_safety=0.95,
    mixed_precision=False, backend="jax", use_z2z=False,
)
...
record.update({
    "windows": n, "gamma_code": gamma, "gamma_gx": gamma * math.sqrt(2.0) / R_OVER_A,
    "drift": float((last.mean() - prev.mean()) / gamma) if prev.size else None,
    "gamma_last_window": float(g[-1]), "growth_every_10th_window": [float(x) for x in g[::10]],
    "wall_s": time.perf_counter() - T0, "first_window_incl_compile_s": t_first,
