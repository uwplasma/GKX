...
Kernels (Cyclone nonlinear deck, complex64 state, cache/params as jit args):
  ...
  window_vjp  jit value_and_grad of nonlinear_heat_flux_window (rk3,
              checkpointed, --window-steps) wrt tprim
Each kernel: one compile call (timed separately), then --reps timed calls,
each forced to host. Records affinity, load averages, XLA_FLAGS and tree.
"""
...
for name, default in (("--Nx", 64), ("--Ny", 64), ("--Nz", 24), ("--Nl", 4), ("--Nm", 8)):
...
cot = jnp.asarray(rng.standard_normal(g0.shape) + 1j * rng.standard_normal(g0.shape), g0.dtype)
dt = float(cfg.time.dt)


def rhs_fn(g, c, p):
    return nonlinear_rhs_cached(g, c, p, terms, compressed_real_fft=True, laguerre_mode="grid")[0]
    ...
    "scan_rk3": (
        jax.jit(
            lambda g, c, p: integrate_nonlinear(
                g, grid, geom, p, dt=dt, steps=args.scan_steps, method="rk3", cache=c,
                terms=terms, compressed_real_fft=True, laguerre_mode="grid",
                return_fields=False,
            )[0]
        ),
        (g0, cache, params),
        ...
        jax.jit(
            jax.value_and_grad(
                lambda tprim: nonlinear_heat_flux_window(
                    g0, grid, geom, replace(params, tprim=tprim), dt, args.window_steps,
                    terms=terms, method="rk3", checkpoint=True, compressed_real_fft=True,
                    laguerre_mode="grid",
                )
            )
        ),
...
for name in [k for k in args.kernels.split(",") if k]:
    ...
    print(name, f"compile {compile_s:.2f}s median {np.median(reps)*1e3:.1f} ms min {np.min(reps)*1e3:.1f} ms", flush=True)
