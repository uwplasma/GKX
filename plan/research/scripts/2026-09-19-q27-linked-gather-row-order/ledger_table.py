"""Print the Q27 full-vs-half ledger: op counts, the byte split, buffer assignment.

usage: ledger_table.py <ledger_dir> [<committed_q10_dir>]

The second argument, when given, is #258's committed ledger directory.  The
``full`` arm and the legacy ``bytes_written`` of *both* arms are then checked
against it, which is what says this row changed the instrument and not the
graph: the compiled modules are the same ones #258 measured.
...
def control(directory: Path) -> None:
    name = "periodic_%s_diagnostics_32.json"
    full = json.loads((directory / (name % "full")).read_text())
    half = json.loads((directory / (name % "half")).read_text())
    print("### chain-free control (boundary = periodic), diagnostics 32")
    header = f"| {'graph':<10} | " + " | ".join(f"{k:>26}" for k in BYTE_KEYS)
    header += f" | {'temp_size_in_bytes':>26} |"
    print(header)
    print("|" + "-" * (len(header) - 2) + "|")
    for label, method in (("RHS", None), ("rk3", "rk3"), ("rk4", "rk4")):
        a, b = counts(full, method), counts(half, method)
        cells = [delta(int(a[k]), int(b[k])) for k in BYTE_KEYS]
        cells.append(
            delta(
                int(memory(full, method).get("temp_size_in_bytes", 0)),
                int(memory(half, method).get("temp_size_in_bytes", 0)),
            )
        )
...
def main(argv: list[str]) -> int:
    ...
    control(directory)
    if len(argv) > 2:
        print("### both arms against #258's committed ledger (counts + bytes_written)")
        ok = check_against_committed(directory, Path(argv[2]))
        print("this row changed the instrument, not the graph:", ok)
    return 0
