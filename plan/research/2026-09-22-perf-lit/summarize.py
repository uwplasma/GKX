...
def forward(path: str) -> None:
    ...
    print(f"== {os.path.basename(path)} shape={r['shape']} state={mb(r['state_bytes'])} MB "
          f"load={r['environment']['loadavg_start'][0]:.0f} dev={r['environment']['devices'][0]}")
    for k, g in r["graphs"].items():
        h = g.get("hlo", {})
        print(f"  {k:20s} warm {g['warm_median_s'] * 1e3:9.3f} ms  compile {g['compile_s']:5.1f} s  "
    ...
    if t and "by_category" in t:
        print("  trace", t["scope"], {k: round(v["frac"], 3) for k, v in t["by_category"].items()})
        for x in t["top"][:8]:
...
def fft(path: str) -> None:
    r = json.load(open(path))
    print(f"== {os.path.basename(path)} fft_s_per_rhs {r['fft_seconds_per_rhs'] * 1e3:.3f} ms  "
...
def arms(path: str) -> None:
    ...
    print(f"== {os.path.basename(path)} {extra} dev={r['environment']['devices'][0]} "
          f"load={r['environment']['loadavg_start'][0]:.0f}")
    for k, a in r["arms"].items():
        keys = ("compile_s", "lower_s", "first_call_s", "warm_median_s", "call_s")
        vals = {x: (round(a[x], 3) if isinstance(a[x], float) else [round(v, 3) for v in a[x]]) for x in keys if x in a}
        print(f"  {k:24s} {vals} temp {mb(a.get('temp_bytes'))} MB dev_peak {mb(a.get('device_peak_bytes'))} MB "
              f"host_peak {mb(a.get('host_peak_bytes'))} MB "
              + " ".join(f"{x}={a[x]:.6g}" for x in ("objective", "grad_tprim_scale", "grad_geometry_norm", "gamma", "grad_norm") if x in a))
