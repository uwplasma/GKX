...
def _block(tree: Any) -> Any:
    ...
    jax.tree.map(
        lambda v: v.block_until_ready() if hasattr(v, "block_until_ready") else v, tree
    )
    return tree
...
def _fft_op(x, *, kind: str, length: tuple[int, ...]):
    """One XLA FFT instruction of the given type over the trailing axes."""

    import jax.numpy as jnp

    axes = tuple(range(-len(length), 0))
    if kind == "RFFT":
        return jnp.fft.rfftn(x, axes=axes)
    if kind == "IRFFT":
        return jnp.fft.irfftn(x, s=length, axes=axes)
    return (jnp.fft.fftn if kind == "FFT" else jnp.fft.ifftn)(x, axes=axes)
...
def _trace_top_ops(fn, args: tuple, trace_dir: Path, calls: int) -> dict[str, Any]:
    ...
    with jax.profiler.trace(str(trace_dir), create_perfetto_trace=True):
        for _ in range(calls):
            _block(fn(*args))
    files = sorted(
        glob.glob(str(trace_dir / "**/*.trace.json.gz"), recursive=True),
    ...
    return {
        "calls": calls,
        "scope": "device" if device else "all processes",
        "total_us": grand,
        "by_category": {
            k: {"us": v, "frac": v / grand if grand else 0.0}
        ...
        "top": [
            {
                "name": k[:120],
                "us": v,
                "frac": v / grand if grand else 0.0,
                "count": counts[k],
...
def build_case(
    nx: int, ny: int, nz: int, nl: int | None, nm: int | None
) -> dict[str, Any]:
...
def cmd_forward(args) -> dict[str, Any]:
    ...
    project = hermitian_projector_for_signature(
        hermitian_projector_signature(
            np.asarray(case["grid"].ky), int(np.asarray(case["grid"].kx).size)
        )
    )
    ...
    def step(G, cache, params):
        ...
        return advance_explicit_nonlinear_state(
            G,
            dG,
            jnp.asarray(args.dt, dtype=real),
            method=method,
            rhs_fn=rhs,
    ...
    graphs = {
        "fields": jax.jit(
            lambda G, c, p: compute_fields_cached(G, c, p, terms=terms).phi
        ),
    ...
    g = rec["graphs"]
    rec["derived"] = {
        "bracket_s_estimate": g["nonlinear_rhs"]["warm_median_s"]
        - g["linear_rhs"]["warm_median_s"],
        "step_over_rhs": g[f"{method}_step"]["warm_median_s"]
        / g["nonlinear_rhs"]["warm_median_s"],
    }
    return rec
...
def cmd_fftfloor(args) -> dict[str, Any]:
    ...
    for s in shapes:
        ...
        kind = s["type"]
        if kind == "RFFT":
            x = jnp.ones(out_shape[: -len(length)] + length, rdtype)
        elif kind == "IRFFT":
            x = jnp.ones(out_shape[:-1] + (length[-1] // 2 + 1,), cdtype)
        else:
            x = jnp.ones(out_shape, cdtype)
        f = functools.partial(_fft_op, kind=kind, length=length)
    ...
    return {
        "command": "fftfloor",
        "source": str(args.forward_json),
        "rows": rows,
        "fft_seconds_per_rhs": total,
        "rhs_seconds": rhs,
        "fft_fraction_upper": total / rhs,
    }
...
def cmd_window(args) -> dict[str, Any]:
    ...
    rec: dict[str, Any] = {
        ...
        "geometry_values": int(sum(v.size for v in geo_vars)),
        "arms": {},
    }
...
def cmd_eigen(args) -> dict[str, Any]:
    ...
    def gamma(geo_vars):
        return solver_growth_rate_from_geometry(
            _with_geometry(geometry, geo_vars),
            n_laguerre=args.nl,
            n_hermite=args.nm,
        )
    ...
    rec: dict[str, Any] = {
        ...
        "Nm": args.nm,
        "Nz": args.nz,
        "operator_size_estimate": n,
        "arms": {},
    }
    rec["arms"]["value"], v = _measure(
        jax.jit(gamma), (geo_vars,), args.repeats, hlo=False
    )
    rec["arms"]["value"]["gamma"] = float(v)
    r, out = _measure(
        jax.jit(jax.value_and_grad(gamma)), (geo_vars,), args.repeats, hlo=False
    )
    r["gamma"] = float(out[0])
    r["grad_norm"] = float(sum(jnp.linalg.norm(x) ** 2 for x in out[1]) ** 0.5)
    rec["arms"]["value_and_grad"] = r
    rec["derived"] = {
        "grad_over_value": r["warm_median_s"] / rec["arms"]["value"]["warm_median_s"]
    }
    return rec
...
def cmd_eigen_adaptive(args) -> dict[str, Any]:
    ...
    for name, fn in (("value", gamma), ("value_and_grad", jax.value_and_grad(gamma))):
        ...
        arm = {
            ...
            "device_peak_bytes": _device_peak(),
        }
        if name == "value":
            arm["gamma"] = float(out)
        else:
            ...
            arm["grad_norm"] = float(
                sum(jnp.linalg.norm(x) ** 2 for x in out[1]) ** 0.5
            )
        rec["arms"][name] = arm
    rec["derived"] = {
        "warm_grad_over_value": rec["arms"]["value_and_grad"]["call_s"][1]
        / rec["arms"]["value"]["call_s"][1]
    }
    return rec
...
def main() -> int:
    ...
    ap.add_argument(
        "command", choices=("forward", "window", "eigen", "eigen-adaptive", "fftfloor")
    )
    ...
    rec = {
        ...
        "window": cmd_window,
        "eigen": cmd_eigen,
        "eigen-adaptive": cmd_eigen_adaptive,
        "fftfloor": cmd_fftfloor,
    }[args.command](args)
    rec = {
        "label": args.label,
        "argv": sys.argv[1:],
