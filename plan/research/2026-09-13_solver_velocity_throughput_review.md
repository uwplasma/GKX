...
## 6. Revised plan (adopted into plan.md §0.5, §2.4, §5.1, §5.3, §5.4)
...
The plan's destination (§1.1) is parity-plus: verified, differentiable,
CPU and GPU. Measured against the codes whose sources were fetched on
2026-09-13 (GX Bitbucket head `3865a5377886`; stella arXiv 1806.02162; GS2
user manual; GENE ParCo 2010 / CPC 2012 / Görler 2011; CGYRO docs; gyaradax
arXiv 2604.06085; GYACOMO GitHub), GKX is ahead on three axes and behind on
three:

Ahead — derivatives (only gyaradax and the iGENE prototype are in the same
class, neither with eigenpair sensitivities or a finite-window transport
VJP); residual-certified eigenpairs (GX and stella have no eigensolver; GENE
and GS2 report SLEPc residuals on transformed operators); and an evidence
ledger regenerated by CI (no other code gates its published numbers this
way).

Behind — (1) **ky ≥ 0 storage**: GX (`Nyc = 1 + Ny/2`), stella and GS2
evolve only the non-negative-ky half and obtain the rest from the reality
condition; GKX evolves both halves in memory (its restart files already
store `Nyc` rows) and rebuilds the negative half twice per stage, which is
the profiled 41.9% and roughly half of the state memory; (2) **implicit
parallel streaming**: stella and GS2 use Kotschenreuther's response matrices
and remove the electron streaming CFL (stella: ≈100× larger stable steps at
kyρ≈1); GX and GKX are explicit; (3) **E×B flow shear**: every comparison
code has it; GKX parks it (§12) behind a stellarator-validation trigger.
Multi-GPU efficiency (GX >75% at four A100s vs 1.26–1.48× on two A4000s)
and f32 throughput are behind as well but follow from the N-workstream.

What "better than the other codes" would concretely mean, and what the plan
now commits to: (a) the ky ≥ 0 layout as a contract with identity, memory
and HLO gates (§5.3 N3), not a micro-optimization; (b) a response-matrix
implicit streaming step whose factorization is shared with the shift-invert
preconditioner and whose derivative is obtained by the implicit-function
theorem — no other code has a differentiable implicit step (§5.4, with an
entry trigger so it is not started from the electrostatic channel where it
does not pay); (c) certified eigenpairs at a cost below GENE's harmonic
Krylov–Schur budget, with the inner-solve statistics published per solve
(§5.1 L1–L6); (d) a declared velocity-regularization contract with ν→0
extrapolation on every collisionless atlas row — GX's shipped collisionless
reference and its docs disagree with its code on exactly this point (§4.3);
(e) time-to-accuracy rows rather than steps per second. E×B shear stays
parked on its existing trigger; it is named in §2.4 as the largest physics
gap for experimental comparison so the deferral is visible.
