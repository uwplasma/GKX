...
5. **GX method paper (Mandell, Dorland et al., J. Plasma Phys. 2024; arXiv:2209.06731)** — GX's own CBC and W7-X benchmarks against GS2/GENE/stella. Since GKX shares GX's Hermite-Laguerre formulation and wout conventions (a_ref = Aminor_p, B_ref = 2|psi_a|/a^2 — identical to stella's GIST convention, confirmed in stella source), these cases triangulate GKX-vs-stella directly: same wout file, same s, same field line, ky grids in the same rho_ref units, times in the same a/v_th units (both codes use v_th = sqrt(2T/m); still worth a one-line unit sanity check in GKX before quoting discrepancies).
