...
class VMEXTransportAdmissionPolicy:
...
@dataclass(frozen=True)
class VMEXNonlinearAuditPolicy:
...
@dataclass(frozen=True)
class VMEXReducedPrelaunchPolicy:
...
@dataclass(frozen=True)
class VMEXNonlinearCampaignPolicy:
...
def _annotated_transport_candidate(
    raw: Mapping[str, Any],
    policy: VMEXTransportAdmissionPolicy,
) -> dict[str, Any]:
    item = dict(raw)
...
def build_transport_admission_report(
    summaries: Sequence[Mapping[str, Any]],
    *,
    policy: VMEXTransportAdmissionPolicy | None = None,
) -> dict[str, Any]:
    ...
    policy = policy or VMEXTransportAdmissionPolicy()
