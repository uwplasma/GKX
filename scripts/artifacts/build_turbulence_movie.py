...
Two-stage use. Physics run, then render; only the two displayed cuts are kept::
    ...
    python scripts/artifacts/build_turbulence_movie.py --render-from movie_cuts.npz \
...
def _check_healthy(phi: np.ndarray, where: str, ceiling: float) -> None:
    ...
    ``scripts/campaigns/nonlinear_saturated_state.py`` -- tau_ac 8.92, window 22.4
    ...
    if peak > ceiling:
        raise RuntimeError(
            ...
            "scripts/campaigns/nonlinear_saturated_state.py rather than guessing. "
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "passes every check in scripts/campaigns/nonlinear_saturated_state.py, "
