...
``tools/release/check_parallel_scaling_artifacts.py`` validates the gate, its
...
- **Device-z pencil bracket.** The ``z``-sharded route keeps both transform
  ...
  ``tools/profiling/profile_device_z_pencil_scaling_decomposition.py``, the
...
They are regenerated by ``tools/artifacts/generate_velocity_parallel_gates.py``
(``hermite-exchange``, ``field-reduce``, ``hermite-ladder``,
``periodic-streaming``), ``tools/artifacts/generate_electrostatic_parallel_gates.py``
(``field-reduce``, ``drift``, ``diamagnetic``) and
``tools/artifacts/generate_linear_rhs_parallel_gates.py`` (``streaming``,
...
They are produced by ``tools/profiling/profile_linear_rhs_parallel_slices.py``
