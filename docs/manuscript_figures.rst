...
This page tracks the target figure set for the future GKX paper. A
...
Render policy: the repository-slimming passes removed reproducible figure
renders (PNG/PDF exports) from git. Figure names on this page therefore refer
to regenerable renders produced on demand by each row's owning script (atlas
panels are additionally recorded under the ``regenerate_on_demand`` action in
``tools/release_artifact_manifest.toml``), while every quoted number is backed
by the machine-readable JSON/CSV evidence tracked in ``docs/_static``.
...
``docs/_static/parallel_decomposition_status.json`` keeps production
...
``docs/_static/nonlinear_gradient_state_control_runbook.json`` is a claim
...
``docs/_static/nonlinear_gradient_state_to_input_mapping_response.json`` is a
negative measured-response record: the current stellarator-symmetric
...
is the positive symmetry-compatible mapping record. The new
...
``docs/_static/vmex_qa_transport_candidate_comparison.json`` (with its CSV
companion; the comparison panel is a regenerable render) is not a
...
.. list-table::
   ...
   * - Linear benchmark master panel
     ...
     - Cyclone ITG, ETG, KBM, W7-X, HSX, and shaped tokamak coverage are represented in the ``benchmark_core_linear_atlas`` panel (a regenerable render under the ``regenerate_on_demand`` manifest action; the tracked inputs are indexed by ``tools/benchmark_atlas_manifest.toml``). TEM/kinetic-electron branch parity and additional W7-X multi-flux-tube scans remain outside the current release claim.
   * - Eigenfunction validation panel
     - ``scripts/artifacts/generate_linear_reference_overlays.py overlap-summary``, ``scripts/artifacts/generate_linear_reference_overlays.py reference-overlay``, ``scripts/artifacts/generate_linear_reference_overlays.py kbm``, and ``scripts/artifacts/generate_linear_reference_overlays.py w7x``
     - Closed for KBM and W7-X raw overlays
     - the overlap-summary panel (``kbm_eigenfunction_overlap_summary``) is a regenerable render; frozen raw GX bundles now exist for KBM and W7-X under ``docs/_static/comparison/reference_modes/``. The closed KBM raw overlay is tracked in ``docs/_static/reference_modes/kbm_eigenfunction_reference_overlay_ky0p3000.json`` with overlap ``0.999985`` and relative ``L^2`` mismatch ``0.00721`` (its overlay figure is a regenerable render). The closed W7-X raw overlay is tracked in ``docs/_static/reference_modes/w7x_eigenfunction_reference_overlay_ky0p3000.json`` with overlap ``0.9999999994`` and relative ``L^2`` mismatch ``3.33e-5`` against the finite GX ``t≈2`` raw-mode bundle (its overlay figure is likewise a regenerable render). Both overlay generators write JSON gate reports with ``overlap >= 0.95`` and ``relative L^2 <= 0.25`` requirements. ``scripts/comparison/compare_gx_kbm.py --branch-summary-json`` writes branch-continuity gate metadata for selected KBM scans, and ``scripts/artifacts/build_linear_validation_artifacts.py kbm-branch`` refreshes the no-rerun tracked artifact ``docs/_static/kbm_branch_gate_summary.json`` from ``docs/_static/comparison/kbm_reference_candidates.csv``. The current continuity-first branch summary passes the strict adjacent growth/frequency jump and successive-overlap gates.
   * - Nonlinear transport panel
     - ``scripts/comparison/make_reference_panels.py summary`` / ``scripts/comparison/make_reference_panels.py publication``
     - Closed for release-window gates; stricter manuscript tightening remains open
     - Cyclone, Miller, KBM, W7-X, HSX with matched windows. Current component figures (regenerable renders, not tracked in git): ``nonlinear_cyclone_diag_compare_t400``, ``nonlinear_cyclone_miller_diag_compare_t122``, ``nonlinear_kbm_diag_compare_t400_stats``, ``nonlinear_w7x_diag_compare_t200``, ``hsx_nonlinear_compare_t50_true``. ``scripts/comparison/compare_gx_nonlinear.py diagnostics --summary-json`` writes machine-readable mean-relative-mismatch gates for each plotted diagnostic with explicit transport-window bounds. The materialized release-window metadata are ``docs/_static/nonlinear_cyclone_gate_summary.json``, ``docs/_static/nonlinear_cyclone_miller_gate_summary.json``, ``docs/_static/nonlinear_kbm_gate_summary.json``, ``docs/_static/nonlinear_hsx_gate_summary.json``, and ``docs/_static/nonlinear_w7x_gate_summary.json``; all pass the current release gate. This is sufficient for the release validation atlas. Paper-level tightening remains open where case-specific references support narrower tolerances, and the older short Cyclone ``t=5`` diagnostic remains documented as an exploratory startup/resolved-spectrum audit, not a release gate.
   * - W7-X exact-state convention audit
     - ``scripts/comparison/build_exact_state_audit.py run`` and ``scripts/comparison/build_exact_state_audit.py report``
     - Closed
     - current artifact base: ``docs/_static/w7x_exact_state_audit.json`` (the panel render and CSV/PDF exports are regenerated on demand and not tracked in git). It compares W7-X nonlinear VMEC startup state, late geometry/field arrays, and re-evaluated scalar diagnostics directly against GX exact-state dumps. The maximum finite pointwise relative error is ``4.62e-5`` under the explicit ``1e-4`` convention gate, while scalar diagnostics are below ``1.8e-7``. This closes the geometry/diagnostic convention layer but does not close the separate W7-X zonal-response recurrence lane.
   * - Windowed-statistics summary
     ...
     - current artifact base: ``docs/_static/nonlinear_window_statistics.json`` (the summary panel and CSV/PDF exports are regenerated on demand and not tracked in git). It summarizes the per-diagnostic ``mean_rel_abs`` and ``max_rel_abs`` statistics from the frozen nonlinear GX comparison gate JSONs for Cyclone, Cyclone Miller, KBM, W7-X, and HSX. Exploratory/short-run diagnostics are explicitly excluded with ``gate_index_include=false``.
   ...
   * - VMEC/Boozer nonlinear startup finite-difference audit
     ...
     - current artifact base: ``docs/_static/vmec_boozer_nonlinear_window_fd_audit.json`` (the audit panel and CSV/PDF exports are regenerated on demand and not tracked in git). It starts from the real mode-21 ``vmex -> booz_xform_jax`` QH state bridge, writes perturbed sampled geometries to temporary NetCDF files, and runs compact nonlinear startup windows at ``Rcos_mid_surface_m1 = base +/- 1e-5`` plus a repeated base point. The gate checks finite outputs, deterministic repeatability, bounded startup-window coefficient of variation and trend, resolved geometry perturbation, and resolved central finite-difference response; response/base is about ``0.040``. Its ``transport_average_gate`` is false and the forward/backward response is asymmetric, so this is a startup observable-path audit rather than a promoted transport average, local nonlinear gradient, or optimized-equilibrium transport claim.

   * - Validation gate index
     - ``scripts/check.py validation-coverage gate-index``
     - Current release-gate audit with quasilinear model-selection deliberately open
     - current artifact base: ``docs/_static/validation_gate_index.json`` and ``docs/_static/validation_gate_index.csv`` (the index panel is a regenerable render). This is not a physics result by itself; it is the audit panel for materialized release-window gates. The current index records ``17/18`` passed: the shaped-pressure external-VMEC high-grid admission gate is included as a passed scoped holdout, while ``docs/_static/quasilinear_model_selection_status.json`` remains open because the required spectral-envelope candidate misses the strict transport-error gate and is not promoted as an absolute-flux predictor.
   * - Quasilinear spectrum panel
     - ``scripts/artifacts/plot_quasilinear_diagnostics.py spectrum``
     - Electrostatic diagnostic closed; absolute-flux prediction not promoted
     - current artifact bases: the tracked scan spectra ``docs/_static/quasilinear_cyclone_spectrum_scan.quasilinear_spectrum.csv``, ``docs/_static/quasilinear_cyclone_miller_spectrum_scan.quasilinear_spectrum.csv``, ``docs/_static/quasilinear_hsx_spectrum_scan.quasilinear_spectrum.csv``, and ``docs/_static/quasilinear_w7x_spectrum_scan.quasilinear_spectrum.csv`` (the spectrum panels and their CSV/JSON/PDF exports are regenerated on demand and not tracked in git). They show electrostatic linear weights and explicitly uncalibrated mixing-length outputs from ``examples/linear/axisymmetric/runtime_cyclone_quasilinear.toml``, ``examples/linear/axisymmetric/runtime_cyclone_miller_quasilinear.toml``, ``examples/linear/non-axisymmetric/runtime_hsx_linear_quasilinear.toml``, and ``examples/linear/non-axisymmetric/runtime_w7x_linear_quasilinear_vmec.toml``. Scan spectra use requested ``ky`` for the x-axis and retain signed selected-mode coordinates as ``mode_ky`` when applicable. W7-X was generated from an external VMEC benchmark file via ``W7X_VMEC_FILE``; the equilibrium itself is not shipped. Absolute saturated-flux claims remain open until a held-out nonlinear calibration report passes.
   * - Quasilinear calibration audit
     - ``scripts/artifacts/plot_quasilinear_calibration.py report`` and ``scripts/artifacts/plot_quasilinear_calibration.py``
   ...
   * - External-VMEC nonlinear holdouts
     - ``scripts/check.py vmec-boozer high-grid-admission``
   ...
   * - Quasilinear spectrum-shape gate
     - ``scripts/artifacts/plot_quasilinear_diagnostics.py shape-gate``
     - HSX, W7-X, and Cyclone Miller gates closed; Cyclone retained as a failed model gate
     - current artifact bases: ``docs/_static/quasilinear_hsx_spectrum_shape_gate.json``, ``docs/_static/quasilinear_w7x_spectrum_shape_gate.json``, ``docs/_static/quasilinear_cyclone_miller_spectrum_shape_gate.json``, and ``docs/_static/quasilinear_cyclone_spectrum_shape_gate.json`` (the shape-gate panels and PDF exports are regenerated on demand and not tracked in git). They compare normalized linear heat-flux-weight spectra against normalized nonlinear resolved ``HeatFlux_kyst`` spectra. HSX passes with ``TV≈0.112`` and cosine ``≈0.971``; W7-X passes with ``TV≈0.056`` and cosine ``≈0.992``; Cyclone Miller passes with ``TV≈0.094`` and cosine ``≈0.983``; Cyclone is kept as a failed gate with ``TV≈0.215`` and cosine ``≈0.896``. This supports spectrum-shape diagnostics while identifying a real saturation/window/model limitation before absolute saturated-flux claims. KBM is deferred from this gate because the current quasilinear diagnostic validates electrostatic channels only and the KBM lane is electromagnetic.
   * - Zonal-flow / GAM response panel
     - ``scripts/artifacts/build_zonal_flow_artifacts.py`` (``response-csv``, ``response-output``, ``objective-gate``, ``miller-panel``, and ``collisional-zonal-dk`` modes), ``scripts/artifacts/build_w7x_zonal_validation_artifacts.py response-panel``, ``scripts/artifacts/build_w7x_zonal_reference_artifacts.py digitize``, ``scripts/artifacts/build_w7x_zonal_reference_artifacts.py compare``, and ``scripts/artifacts/build_w7x_zonal_validation_artifacts.py`` (``contract`` and ``state-convention`` modes)
     - Open
     - The complete P24/J10 Coulomb, original-Sugama, and improved-Sugama Figures 12--14 protocol is closed in ``docs/_static/collision_finite_wavelength_zonal_response.json`` with the compact velocity-section companion ``docs/_static/collision_finite_wavelength_zonal_velocity_sections.csv`` (the response figure is a regenerable render): all traces reach ``t nu=30``, pass the Xiao residual and finite-wavelength tail-ordering gates, and reproduce the published velocity-section relationships. The broader stellarator-zonal panel remains open: it should combine shaped-tokamak Rosenbluth-Hinton-style residuals with W7-X residual/damping envelopes using one figure convention; use signed ``Phi_zonal_mode_kxt`` or case-specific signed line averages for publication claims and keep ``Phi2_zonal_t`` only as an intermediate cross-check. The current Merlo Case-III artifact is ``docs/_static/miller_zonal_response_pilot.json`` (its panel render is regenerated on demand) from the initial-density setup at ``Nz=32``, ``Nl=4``, ``Nm=144``, ``dt=0.0025``, and ``t=60 a/v_i``, which is the **converged** protocol rather than the resolution that agreed with the paper. Two conditions fix it and both are gated in the companion JSON: velocity-space recurrence sets in at ``t_quiet ≈ 5.5 sqrt(Nm)``, so the residual window ``t ∈ [42, 60]`` needs ``Nm ≳ 120``; and the Hermite streaming CFL scales as ``sqrt(Nm)``, so ``Nm=144`` needs ``dt ≤ 0.0025`` (it goes non-finite at ``t=46.5`` with the old ``dt=0.005``). The retired ``Nm=24`` baseline agreed with the read-off only because its whole averaging window sat past recurrence onset, where the scatter inside the window (``1.20`` of the gated value) exceeded the number being gated. With Rosenbluth-Hinton first-sample normalization the converged run gives ``residual = 0.206 ± 0.006`` against the Merlo et al. Figs. 12/16 read-off of about ``0.19`` (gap ``0.016``, paper-scale tolerance ``0.015``); Hilbert-phase frequency extraction gives ``ω_GAM R0 / v_i = 2.345 ± 0.05`` against ``2.24`` (gap ``0.105``, tolerance ``0.10``); and a sliding one-period-RMS envelope fit over three GAM periods gives ``γ_GAM R0 / v_i = -0.184 ± 0.010`` against ``-0.17`` (gap ``0.014``, tolerance ``0.03``). **So the residual and the frequency do not agree with the published values at converged resolution, and the tolerances have not been widened to make them.** The artifact's asserted ``gate_report`` pins GKX's converged numbers and the recurrence/window/trace-completeness conditions behind them; the paper comparison sits in ``literature_comparison`` with ``paper_scale_gate_passed: false``. Publication claims for this case must therefore be scoped to GKX's converged value with its uncertainty and an explicit statement of the gap, not to agreement with Merlo. Carrying Merlo Table III's dropped ``α_MHD = 0.5425`` into ``betaprim = -0.1012`` was tested and reproduces the baseline trace bit for bit, because ``betaprim`` only moves metric and drift coefficients that multiply ``k_y`` and this benchmark runs at ``k_y = 0``. The W7-X side now uses the potential initializer, signed line-average observable, paper-facing line-first normalization, and no hidden time-axis scaling. The tracked long-window W7-X artifact is ``docs/_static/w7x_zonal_response_panel.json`` with replayable traces in ``docs/_static/w7x_zonal_response_panel.traces.csv`` (the panel render is regenerated on demand and not tracked in git); it reaches the digitized Fig. 11 windows, but ``docs/_static/w7x_zonal_reference_compare.json`` remains open because residuals fail at ``k_x rho_i=0.07``, ``0.10``, and ``0.30`` and the late envelopes are much larger than the digitized stella/GENE traces. ``docs/_static/w7x_zonal_contract_audit.json`` is now the paper-facing diagnostic record for that open mismatch and is intentionally excluded from the release gate index (its panel is a regenerable render). ``docs/_static/w7x_zonal_state_convention_audit.json`` closes the paper-facing state convention layer (again with a regenerable panel render): the recovered Gaussian potential has relative ``L2`` error ``1.85e-6``, off-target spectral content is zero to reported precision, and the diagnostic helpers agree with manual line/volume reductions near ``2e-16``. ``docs/_static/w7x_zonal_closure_ladder_kx070.json``, ``docs/_static/w7x_zonal_recurrence_sweep_kx070.json``, and ``docs/_static/w7x_zonal_hypercollision_probe_kx070.json`` are companion open diagnostics (their panels, like the ``w7x_zonal_moment_tail_audit`` panel, are regenerable renders not tracked in git); together they support a recurrence / moment-closure hypothesis and show that weak or constant closure can reduce velocity-space tails without closing the paper trace. The refreshed closure ladder now covers constant Hermite, ``k_z``-weighted Hermite, mixed Laguerre-Hermite, Laguerre-only, and isotropic hypercollision families at ``0.01`` and ``0.03``. The best mean trace error is the isotropic ``nu_hyper=0.01`` row at about ``0.2755`` versus baseline ``0.2861``, but its late-window standard-deviation ratio is about ``4.25`` versus baseline ``4.10``. Thus no bounded closure family improves trace error, late-envelope recurrence, and moment-tail metrics simultaneously. The W7-X generator exposes explicit hypercollision and Gaussian-width audit overrides so future closure probes are reproducible from the tracked tool. A newer high-moment four-wavelength audit under ``tools_out/zonal_response/w7x_publication_nl16_nm64_dt005_t100`` verifies finite signed traces to ``t≈100`` after restart-continuation fixes. The tracked W7-X TOML keeps ``gaussian_width=1`` because the benchmark source writes the initializer as ``exp[-(z-z0)^2]``; wider profiles and non-unit time scales are retained only as audits. The lane remains open pending a more physical W7-X damping/closure and velocity-space recurrence fix under the paper-facing convention.
   * - W7-X fluctuation-spectrum panel
     - ``scripts/artifacts/plot_w7x_fluctuation_spectrum_panel.py`` and ``scripts/artifacts/build_tem_validation_artifacts.py w7x-extension``
     - Initial simulation-spectrum diagnostic closed; TEM/multi-flux validation open
     - current artifact bases: ``docs/_static/w7x_fluctuation_spectrum_panel.json``, ``docs/_static/tem_branch_parity_audit.json``, and ``docs/_static/w7x_tem_extension_status.json`` (the panels and their CSV/PDF exports are regenerated on demand and not tracked in git). The fluctuation panel is regenerated from the gated W7-X nonlinear ``t≈200`` NetCDF artifact, requires the corresponding nonlinear gate summary to pass before plotting, and records ``gate_index_include=false`` because it is a diagnostic figure rather than an additional release gate. It shows normalized ``k_y`` spectra for ``|\phi|^2``, ``W_\phi``, and ``|Q_i|``, the time-averaged ``k_x``-``k_y`` fluctuation-power map, the signed heat-flux spectrum, and a windowed temporal spectrum for the dominant nonzonal and zonal traces. The TEM audit explicitly keeps TEM linear parity open: maximum absolute relative growth-rate mismatch is about ``4.25``, maximum absolute relative frequency mismatch is about ``3.3`` away from the near-zero reference denominator, and the frequency branch has Spearman coefficient about ``-0.986``. Because the TEM reference is a provisional literature digitization rather than a direct case dump, this artifact blocks broad W7-X/TEM validation claims without being a standalone tuning target. The extension-status panel also keeps W7-X multi-alpha/multi-surface scans and kinetic-electron nonlinear windows open. This closes the reproducible simulation-spectrum panel needed for the current manuscript stack but not broad W7-X/TEM validation.
   * - Velocity-space convergence panel
     - ``scripts/artifacts/build_linear_validation_artifacts.py observed-order`` plus dedicated full convergence refresh script to add
     - Open
     - should follow observed-order convergence evidence and write an observed-order gate report through ``gkx.diagnostics.validation_gates.observed_order_gate_report`` so convergence rate and final-grid error are tracked explicitly. The current atlas summary already records a machine-readable high-vs-low Cyclone grid convergence gate for the tracked convergence tile. The CSV-backed Cyclone velocity-space artifact is ``docs/_static/cyclone_resolution_observed_order.json`` (the convergence figure is a regenerable render); the current office/GPU ``ky=0.30`` sweep passes the strict pairwise-order and final-error gate.
   ...
   * - Performance panel
     ...
     - current artifact bases: ``docs/_static/runtime_memory_benchmark.png``, ``docs/_static/nonlinear_rhs_profile_miller.json``, ``docs/_static/nonlinear_rhs_profile_stellarator_runtime.json`` (the two profiler panels are regenerable renders), ``docs/_static/full_nonlinear_rhs_trace_summary.json``, ``docs/_static/full_nonlinear_rhs_trace_gpu_summary.json``, ``docs/_static/nonlinear_sharding_profile_office_gpu_benchmark_grid.json``, and ``docs/_static/nonlinear_device_z_pencil_transport_gpu2_observable_split_profile.json``. The release claim is limited to current cold/warm runtime accounting, CPU/GPU nonlinear RHS hot-path localization, W7-X/HSX runtime-mode stellarator smoke profiles, and fail-closed numerical-identity gates. Whole-state sharding fails identity on the benchmark grid, and the device-z pencil route remains below the two-GPU speedup gate, so neither is a production nonlinear domain-decomposition speedup claim.
   * - Parallelization identity gate
     ...
     - current artifact base: ``docs/_static/parallel_ky_scan_gate.json`` (the gate panel and CSV/PDF exports are regenerated on demand and not tracked in git). This is a real linear-solver gate: it compares serial and fixed-shape ``k_y``-batched Cyclone scans, requires numerical identity for ``gamma`` and ``omega``, and reports batch speedup separately from the acceptance criterion.
...
.. list-table::
   ...
   * - Sensitivity-analysis figure
     ...
     - current artifact base: ``docs/_static/quasilinear_implicit_sensitivity.json`` (the sensitivity panel and PDF export are regenerated on demand and not tracked in git). It differentiates a tiny Cyclone linear-RHS quasilinear objective ``[gamma, omega, kperp_eff^2, Qhat_i, Q_i^ML]`` with respect to ``[a/Ln, a/LTi]`` using the implicit left/right eigenpair system and checks the result against nearest-branch central finite differences. This is a differentiability/conditioning gate, not an absolute nonlinear-flux validation.
   * - Solver-objective geometry-gradient gate
     - ``scripts/artifacts/build_solver_objective_gradient_gate.py``, ``scripts/artifacts/build_solver_objective_gradient_gate.py vmec-boozer frequency``, ``scripts/artifacts/build_solver_objective_gradient_gate.py vmec-boozer quasilinear``, ``scripts/artifacts/build_solver_objective_gradient_gate.py vmec-boozer nonlinear-window``, ``scripts/artifacts/build_vmec_boozer_gradient_holdout_matrix.py``, ``scripts/artifacts/build_vmec_boozer_aggregate_objective_gate.py multi-point``, ``scripts/artifacts/build_vmec_boozer_aggregate_objective_gate.py line-search-comparison``, ``scripts/artifacts/build_vmec_boozer_aggregate_holdout_gate.py alpha``, ``scripts/artifacts/build_vmec_boozer_aggregate_holdout_gate.py surface``, ``scripts/artifacts/build_vmec_boozer_aggregate_objective_gate.py second-equilibrium``, ``scripts/check.py vmec-boozer aggregate-holdout``, ``scripts/check.py nonlinear-optimization production-guard``, ``scripts/artifacts/build_nonlinear_window_fd_audit.py``, and ``scripts/artifacts/build_vmec_boozer_nonlinear_window_fd_audit.py``
     - Solver-ready linear-RHS gradient gate closed; mode-21 VMEC/Boozer state-to-solver eigenfrequency, quasilinear heat-flux-weight, and reduced nonlinear-window estimator gates closed for QH plus Li383; nonlinear startup FD audits remain plumbing checks; production nonlinear optimization remains unpromoted because the aggregate summaries lack auditable raw, stationarity, resolution, and spectral evidence
     - current artifact bases (tracked machine-readable evidence; the corresponding panels are regenerable renders not tracked in git): ``docs/_static/solver_objective_gradient_gate.json``, ``docs/_static/vmec_boozer_solver_frequency_gradient_gate.json``, ``docs/_static/vmec_boozer_quasilinear_gradient_gate.json``, ``docs/_static/vmec_boozer_nonlinear_window_gradient_gate.json``, ``docs/_static/vmec_boozer_li383_nonlinear_window_gradient_gate.json``, ``docs/_static/vmec_boozer_gradient_holdout_matrix.json``, ``docs/_static/vmec_boozer_multi_point_objective_gate.json``, ``docs/_static/vmec_boozer_aggregate_line_search_comparison.json``, ``docs/_static/vmec_boozer_aggregate_alpha_holdout_gate.json``, ``docs/_static/vmec_boozer_aggregate_surface_holdout_gate.json``, ``docs/_static/vmec_boozer_second_equilibrium_aggregate_gate.json``, ``docs/_static/vmec_boozer_holdout_transport/vmec_boozer_qh_torflux078_alpha120_holdout_ensemble_gate.json``, ``docs/_static/vmec_boozer_aggregate_holdout_promotion_gate.json``, ``docs/_static/production_nonlinear_optimization_guard.png``, ``docs/_static/optimized_equilibrium_replicates/optimized_equilibrium_replicate_t700_ensemble_gate.json``, ``docs/_static/qa_projected_transport_step1e3_redesign_report.json`` (its matched-comparison panel is a regenerable render), ``docs/_static/strict_qa_top12_edge_prelaunch_gate.json``, ``docs/_static/strict_qa_top12_edge_matched_nonlinear_transport.json``, ``docs/_static/strict_qa_top12_edge_redesign_report.json``, ``docs/_static/strict_qa_rerun_baseline_ensemble_gate.json``, ``docs/_static/strict_qa_top12_step1p25em3_candidate_ensemble_gate.json``, ``docs/_static/nonlinear_window_fd_audit.json``, and ``docs/_static/vmec_boozer_nonlinear_window_fd_audit.json`` with CSV/PDF companions where available. The linear and reduced-objective artifacts close their stated derivative and holdout gates. Historical matched summaries report ``18.4%``, ``2.68%``, and ``3.35%`` reductions, but lack hashed raw sources and explicit stationarity, uncertainty, resolution, and spectral gates; none is production promotion evidence. Earlier candidate transfers remain negative. The compact nonlinear FD audits verify startup response only. Nonlinear turbulence-gradient and multi-surface optimization claims remain open.
   * - Historical strict-QA ``t=1500`` summaries
     - ``scripts/check.py nonlinear-transport runtime-outputs``
     - Not independently auditable; not promotion evidence
     - The JSON summaries claim ``t=1500`` and ``t=[1100,1500]``, but their referenced office NetCDF outputs are unavailable and the retained generated trace CSVs end near ``t=400``. The recorded comparisons are negative (growth ``0.60%``, quasilinear ``-0.49%``, nonlinear-window ``-0.25%``); the panels are historical summaries, not robust-trace evidence.
   * - Inverse/UQ figure
     - ``examples/theory_and_demos/autodiff_inverse_growth.py``, ``examples/theory_and_demos/autodiff_inverse_twomode.py``, and ``scripts/artifacts/plot_stellarator_optimization_uq.py``
     - Scoped inverse and UQ validation closed; global identifiability and production optimization claims remain scoped
     - current artifact bases: ``docs/_static/autodiff_inverse_growth_summary.json``, ``docs/_static/autodiff_inverse_twomode.png`` with ``docs/_static/autodiff_inverse_twomode_summary.json``, and ``docs/_static/stellarator_itg_optimization_uq.json``, with CSV sweep companions where available (the ``autodiff_inverse_growth`` and UQ panels are regenerable renders not tracked in git). The inverse examples check AD/finite-difference consistency and conditioning for one- and two-parameter reduced objectives, while the UQ panel reports local Gauss-Newton covariance, rank, and sensitivity-map diagnostics. These figures validate the differentiable workflow and uncertainty reporting, not global uniqueness or production nonlinear stellarator optimization.
   * - Solved VMEC-JAX QA geometry figure
     - upstream ``vmex`` ``QA_optimization.py`` workflow plus local panel
     ...
     - current artifact base: the ``vmex_qa_solved_boundary_boozer_panel``
       figure, a regenerable render that is no longer tracked in git (it was
       removed with the other reproducible renders in the repository-slimming
       passes and is rebuilt from the stated owning workflow). The
   ...
   * - Development-only optimization-plumbing figure
     - ``examples/theory_and_demos/reduced_stellarator_itg/compare_stellarator_itg_optimizations.py`` and ``scripts/artifacts/plot_stellarator_optimization_uq.py``
     ...
     - primary artifact base: ``docs/_static/stellarator_itg_optimization_uq.json`` with ``docs/_static/stellarator_itg_nonlinear_optimization.json`` (the UQ panel and the individual ``stellarator_itg_growth_optimization``, ``stellarator_itg_quasilinear_optimization``, and ``stellarator_itg_nonlinear_optimization`` diagnostic panels are regenerable renders not tracked in git). The supporting reduced comparison sidecar ``docs/_static/stellarator_itg_optimization_comparison.json`` records objective histories and reduction ratios, but its companion PNG is a synthetic reduced max-mode-1 surface diagnostic and is not a solved-geometry optimization figure. These files live under ``examples/theory_and_demos/reduced_stellarator_itg`` rather than ``examples/optimization``. The UQ panel computes covariance from the final weighted objective residual Jacobian and shows derivative parity, control uncertainty, covariance projection, and rank/conditioning diagnostics. The production QA optimization examples are the VMEC-JAX-style scripts in ``examples/optimization``; they remain separate from this diagnostic artifact stack and still require solved-WOUT gates plus converged nonlinear audits before transport-optimization claims.

Quasilinear Model-Selection Figures
-----------------------------------

These quasilinear model-development results are retained as static evidence;
their generator scripts were retired. The machine-readable JSON companions
remain tracked and are the artifacts of record; the compressed panels
themselves (except the still-tracked
``docs/_static/quasilinear_stellarator_usefulness.png``) were removed from git
in the repository-slimming passes and, because the generators were retired,
...
- explain, or scope the claim for, the residual and GAM-frequency gap between
  GKX's converged Merlo Case-III values and the published read-off. Both are
  marginal (``1.06x`` and ``1.05x`` the paper-scale tolerance) and both grow
  monotonically with Hermite resolution, so refinement does not close them; the
  dropped ``α_MHD`` has been ruled out because it only enters ``k_y``-weighted
  coefficients. Remaining candidates are the figure read-off accuracy itself
  (Fig. 16 supports ``ω_GAM`` nearer ``2.25--2.30``), the dropped squareness
  ``ζ = 2.83e-3``, and the fact that the paper runs to ``150 R0/v_i`` while a
  collisionless Hermite closure cannot reach that horizon free of recurrence at
  any feasible ``Nm``
