"""Stellarator ITG objectives: facade, optimizer loop, tables, and contracts.

This module absorbed ``stellarator_tables``, ``stellarator_contracts`` and
``stellarator_reduced``; each had a single consumer that imported most of its
names. The reduced-model deprecation notice below came from the last of those
and is reproduced verbatim, because it bounds what the reduced observables
...
from gkx.objectives.portfolio import objective_portfolio_sensitivity_report
...
# Constants fused from stellarator_contracts.py. They sit above their first
# use rather than at the end of the file: several appear in DEFAULT ARGUMENTS,
# which evaluate at definition time, so appending them would raise NameError.
...
# ---- fused from stellarator_tables.py ----
# One consumer, and it imported most of these names. That is a split
# module rather than an interface, so the two are one file again.
...
def stellarator_itg_vmec_boozer_sample_objective_table_from_state(
    ...
    **vmec_boozer_options: Any,
    ...
    """Return real VMEC/Boozer/GKX rows on a ``StellaratorITGSampleSet``.
...
def stellarator_itg_vmec_boozer_portfolio_objective_from_state(
    ...
    **vmec_boozer_options: Any,
    ...
    """Reduce real VMEC/Boozer/GKX ITG rows to one portfolio scalar."""
    ...
    *,
    ...
    return {
        ...
        "next_action": (
            ...
            "booz_xform_jax -> GKX rows, then rerun the same gate "
...
# ---- fused from stellarator_reduced.py ----
# One consumer, and it imported most of these names. That is a split
# module rather than an interface, so the two are one file again.
...
# ---- fused from stellarator_contracts.py ----
# One consumer, and it imported most of these names. That is a split
# module rather than an interface, so the two are one file again.
