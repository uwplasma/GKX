...
class SAlphaGeometry(_SpectralGeometryMixin):
    ...
    def metric_coeffs(
        ...
        # At exact zero shear it uses kx directly, so gds22 must be 1 rather than
        # s_hat**2; leaving it at zero erases the radial-mode FLR dependence.
        # This is a distinct coordinate contract shared with SlabGeometry;
        # derivatives across its zero-shear switch are intentionally not claimed.
        s_hat = jnp.asarray(self.s_hat)
        gds22 = jnp.where(s_hat == 0.0, jnp.ones_like(theta), s_hat * s_hat)
...
class SlabGeometry:
    ...
    def metric_coeffs(
        ...
        s_hat = jnp.asarray(self.s_hat)
        gds22 = jnp.where(s_hat == 0.0, jnp.ones_like(theta_arr), s_hat * s_hat)
        return gds2, gds21, gds22
